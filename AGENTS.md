...
- Check `npm run dev:session -- status` (or `pgrep -fl vite`) for the one shared server at <http://localhost:5173>. Agents never start their own — see the single-server rule above.
