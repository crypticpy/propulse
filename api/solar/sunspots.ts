/**
 * Vercel Edge Function: Sunspots Proxy
 * Fetches sunspot number data from NOAA SWPC to avoid CORS restrictions
 *
 * Source: https://services.swpc.noaa.gov/json/solar-cycle/sunspots.json
 * Cache: 24 hours with 2 hour stale-while-revalidate
 */
...
export default async function handler(request: Request): Promise<Response> {
  const limited = applyRateLimit(request, "solar/sunspots", 30, 60);
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: `Failed to fetch sunspot data: ${message}`,
      }),
      {
        status: 500,
        headers: {
