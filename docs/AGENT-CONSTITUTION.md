...
- **What counts as UI**: anything a person sees. The merger decides, and when
  ...
  modules, layout and presentation hooks such as `useHomeLayout` and `useDisplayFit`, presentation stores such as `hamclockDisplayStore`, `kioskStore` and `workspaceStore`, the root shell `src/App.tsx`/`src/main.tsx`/`index.html`, `public/`,
