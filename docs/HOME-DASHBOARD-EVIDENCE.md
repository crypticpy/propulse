...
- Optional panels render in place on the dashboard. Moon, DXpeditions, and world clocks are visible by default; an “Add more panels +” disclosure adds the rest with one click. Customize dashboard opens the ordered layout for drag, ▲ ▼, Hide, and Reset to default, saved separately for desktop/tablet and phone under `propulse-home-layout-v2`. Personal station/log details have a separate disclosure.
