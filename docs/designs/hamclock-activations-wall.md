...
- Report matrix: all three programmes × Pulse/Classic/Brass × 1920×1080 and 3840×2160. No report, panel or list overflow. Tile containment is checked under the same theme/resolution changes with a populated neighbouring cluster. A visually hidden wrapper contains the table's intrinsic height. Row measurement observes the rows as well as their slot, so theme/font/wrapping changes cannot leave a previously fitted row clipped.
