...
Release validation on 2026-09-07 merged the slice cleanly onto current `main`.
The 12 focused handler tests, repository lint, and production build passed. A
read-only query using the deployed public anonymous configuration confirmed that
`band_hourly_stats` was accessible and contained a numeric aggregate for the
latest completed UTC hour. Live data presence establishes the prerequisite only;
the endpoint and report UI still require their normal deployment steps.
