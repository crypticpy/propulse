...
- Next #288 slice: DX Cluster list source-history prerequisite, `.worktrees/hamclock-cluster-history`, branch `feat/hamclock-cluster-history`, based on #472. Existing list age choices include 5 and 120 minutes while REST defaults to 30. Add bounded, DX-only two-hour source/client support backed by the existing retention contract; keep PSK/RBN and global map choices at 15/30/60. Own source window parser/store/handler tests, browser metadata/DX client types/tests and evidence. Subsequent list-hook slice must retain metadata/errors, separate valid empty from failure, advance expiry on the clock, and preserve bridge ownership. No production migrations, collector, renderer or modeling changes claimed.
