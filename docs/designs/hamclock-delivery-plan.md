...
- Next #288 slice: `.worktrees/hamclock-spot-density`, branch `feat/hamclock-spot-density`, based on #426. Own `mapStore` density/default normalization, `spotDensity` helpers/tests, new wall settings `SpotsTab`, settings dialog registration/test and evidence documentation. Measure complete flat rendering at 50/150/200 before raising the default. Preserve the 10–200 bound and analysis-feed floor. Shared age-window/source controls and PSK personal-map integration follow separately; global sampled-history limits remain explicit. No 3D internals or modeling files are claimed. #287's map acceptance continues here without closing either issue prematurely. The geometry fixture server/browser have been stopped and its exact stale claim verified/removed.
