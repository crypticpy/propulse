...
| --- | --- | --- |
| Codex / HamClock operating views | Current implementation: coordinated spot display #288; PSK/WSJT-X #287, Activations #285, shared tuning #286 and B24 #232 in review | One active implementation item; retain review and acceptance follow-up |
...
- #288 is the sole active Codex implementation claim. First slice: true great-circle flat spot paths and circle=TX/square=RX endpoints, including highlighted paths, with `drawCallsignLabels` preserved. Following slices: validated density defaults/segmented controls and a shared age-window/source contract, including PSK's callsign-specific 24-hour source. Global sampled-feed history limitations must remain explicit. Worktree `.worktrees/hamclock-spot-display`, branch `feat/hamclock-spot-display`, starts from #424. Initial file ownership is `FlatMapView.tsx` plus a small flat-path helper/test; shared mapStore, feed hooks/endpoints and HamClock settings changes are serialized afterward. 3D internals and NowCast/modeling remain with their owner. Claim: [issue comment](https://github.com/crypticpy/propulse/issues/288#issuecomment-5563741698). #287's map acceptance continues here without closing either issue prematurely.
