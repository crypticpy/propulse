...
PSK source PR #422 merged as `25cf48e7` and its production deployment succeeded. One anonymous endpoint smoke returned a short non-JSON response, so deployed feed behavior is not yet verified; diagnosis remains open. Report PRs #423/#424 have been refreshed on the merged prerequisites with exact heads `9b44dd20` and `991ae9f3`, respectively (15 and 10 files). Their reviewed report behavior is unchanged; focused tests and full pre-push gates pass.
