...
| --- | --- | --- |
| Codex / HamClock operating views | Current implementation: PSK Reporter / WSJT-X #287; Activations #285, shared tuning #286 and B24 #232 in review | One active implementation item; retain review and acceptance follow-up |
...
- #287 is the sole current implementation claim. Start with decode-time dial frequency/mode and source identity, then WSJT-X wall surfaces, PSK OF/BY-call and age/band windows, and coordinated map-window integration. Initial file ownership: WSJT-X bridge/client message types, ingestion, wsjtxStore and tests. Longer PSK windows require a verified source; never infer old decode frequency from the current dial. #288 stays unclaimed until the coordinated display slice is ready. Claim: [issue comment](https://github.com/crypticpy/propulse/issues/287#issuecomment-5563130138).
- #287 delivery: [#419](https://github.com/crypticpy/propulse/pull/419) captures per-instance decode-time dial context; [#420](https://github.com/crypticpy/propulse/pull/420) wires lossless direct/extension ingestion and corrects frequency/date consumers; [#421](https://github.com/crypticpy/propulse/pull/421) adds the WSJT-X wall tile/report, explicit captured-dial tuning and v6 SDR migration. [#422](https://github.com/crypticpy/propulse/pull/422) adds the bounded callsign-specific 24-hour PSK snapshot, five-minute request reuse/cooldown, exact Hz and honest success/empty/stale/unavailable states. The global collector uses a 1% sample; this separate source answers OF/BY-call queries while labeling loaded-row limits rather than promising complete history. The mixed native/bridge timestamp review on #420 is fixed and regression-tested across midnight, with the correction propagated through #421/#422. Latest full gates: 376 files / 3,288 tests, lint, build, bundle budgets and bridge build; bridge suite 27 tests; wall browser matrix remains 36 cases at 1080p/4K with hardware blocked. [#423](https://github.com/crypticpy/propulse/pull/423) adds the PSK tile/report, shared OF/BY/band/window controls, explicit tuning, pinning and v8 custom-preserving rail migration. Full gates pass: 378 files / 3,295 tests, lint, build, bundle budgets and bridge build; 78 browser report cases plus tile bounds/pinning/focus at 1080p/4K pass with hardware blocked. Evidence: `docs/designs/hamclock-psk-wall.md` in its branch. #422's quota/capacity/parser review findings are addressed in `b8896ccb`: service-role-only Postgres claim/publication RPCs coordinate every caller and instance, snapshots are shared, parsing retains at most 1,000 objects, and unavailable coordination fails closed. The isolated, network-disabled PostgreSQL gate passes permissions, reuse, cooldown, fencing, capacity, migration reapplication and ten-client concurrency with one grant. Full repository verification passes, including Python, frontend, bridge and daemon suites, build and bundle checks (endpoint: 377 files / 3,297 frontend tests; dependent UI: 379 files / 3,306 after the migration-order follow-up). The database migration is **not applied in production**; its manual application and server cache credentials remain explicit maintainer deployment prerequisites. #423's direct-v6/v7 migration-order review has ascending steps and 44 passing focused tests in `f276b3ff`. Current next slice is WSJT-X heard-by integration in the clean `.worktrees/hamclock-wsjtx-heard`, aligned with the reviewed PSK base. Coordinated map windows/glyphs remain outstanding.
