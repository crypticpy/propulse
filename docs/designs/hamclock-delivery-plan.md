...
- Next #288 slice: personal PSK map scope, `.worktrees/hamclock-psk-map`, branch `feat/hamclock-psk-map`, based on #471. Follow #287's direction/band/15–1440-minute report controls through the existing bounded callsign-specific snapshot; preserve each receiver path, original timestamps and policy restrictions. Explicitly distinguish personal PSK from global sampled traffic. Own lightweight map feed/resolver boundary, map setting, personal report/settings controls, pure conversion helper/tests and evidence, serialized into ≤15-file PRs if needed. No collector/database/3D/model internals claimed. DX Cluster's separate list age controls remain a subsequent integration item.
