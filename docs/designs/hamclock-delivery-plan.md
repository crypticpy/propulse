...
- Next #288 slice: `.worktrees/hamclock-cluster-feed`, branch `feat/hamclock-cluster-feed`, based on #474. Wire DX metadata/client history to existing 5/15/30/60/120 age choices with scoped queries, clock expiry, original source times, cached failure versus valid empty, and passive tile/report source states. Own `useDXCluster`/tests, pure history/state helper/tests if needed, dxStore, DXSpotList state/list, ClusterTile/ClusterReport/tests and evidence, ≤15 files. Disabled/external-filter consumers must not overwrite the default shared snapshot; preserve bridge ownership and newly mounted empty-observer behavior. No collector/database/renderer/model edits; weather last.
