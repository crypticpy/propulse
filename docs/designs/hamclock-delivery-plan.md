...
- Next #288 slice: integrate the existing #410 Cluster report into the active stack before changing list behavior. Worktree `.worktrees/hamclock-cluster-report-integration`, branch `feat/hamclock-cluster-report-integration`, based on #473, locally merging `feat/hamclock-b24-cluster` (`2da274a9`). Preserve newer shared report styles/row helpers and tracker entries; remain within #410's 15-file footprint, using its existing evidence document/image. Verify Cluster and PSK together. This is a reviewable integration PR, not a GitHub merge; maintainer merge policy remains unchanged. After integration, wire list metadata/window/error/empty/expiry behavior while preserving bridge ownership. No new issue claim or collector/database/renderer/model work.
