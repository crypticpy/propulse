...
- Next #288 slice: `.worktrees/hamclock-spot-age-client`, branch `feat/hamclock-spot-age-client`, based on #469. Own browser PSK/RBN/DX Cluster API clients and focused tests plus a small shared metadata/type helper. Preserve window/freshness metadata and existing array callers, then wire shared map/cluster hooks and Settings age selection with clock-based expiry. Keep unavailable/stale state and capped sample coverage explicit. Global 6h/24h remains unsupported; personal PSK map scope follows separately. No collector/database/3D/model internals are claimed.
