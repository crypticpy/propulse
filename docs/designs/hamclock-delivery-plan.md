...
- Next #288 slice: `.worktrees/hamclock-spot-age-map`, branch `feat/hamclock-spot-age-map`, based on #470. Own mapStore age setting/tests, `useLiveSpots`, shared `useResolvedMapSpots` and tests, a small map age/expiry helper, Settings `SpotsTab` and evidence docs/screenshots. Add map-specific 15/30/60 choices, clock-based expiry on original timestamps, source metadata and trace-scope fidelity. Keep 3D internals and callsign placement untouched. DX Cluster's separate 5/15/30/60/120 list controls need a separate integration follow-up; do not silently coerce them into the map options. Personal PSK map scope follows separately. No collector/database/model internals are claimed.
