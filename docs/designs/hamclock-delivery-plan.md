...
- #287 delivery: [#419](https://github.com/crypticpy/propulse/pull/419) captures per-instance decode-time dial context; [#420](https://github.com/crypticpy/propulse/pull/420) wires lossless direct/extension ingestion and corrects frequency/date consumers; [#421](https://github.com/crypticpy/propulse/pull/421) adds the WSJT-X wall tile/report, explicit captured-dial tuning and v6 SDR migration. Latest full gates: 374 files / 3,276 tests, lint, build, bundle budgets; bridge suite 27 tests; wall browser matrix 36 cases at 1080p/4K with hardware blocked. Current slice is PSK station data: the global collector uses a 1% sample, so OF/BY-call evidence needs a complete callsign-specific source. Official retrieval supports up to 24 hours and requests no more often than five minutes. Cache/refresh design must enforce that guidance; PSK views and heard-by integration remain outstanding.
