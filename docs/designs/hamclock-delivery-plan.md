...
- Next #288 slice: bounded Cluster bridge-ingestion audit/fix in `.worktrees/hamclock-cluster-ingest`, branch `feat/hamclock-cluster-ingest`, based on #475. The hook still consumes `lastMessage` through an effect and keeps per-observer history, while `useBridge` already provides a synchronous per-message callback. First reproduce burst loss and new-observer first-message history replacement; if confirmed, use direct ingestion and a deduplicated shared default snapshot while preserving isolated external filters and bridge ownership. Own hook/tests, a small pure merge/validation helper/tests if needed, and evidence docs; no renderer/model/collector/hardware service changes or extra issue claim. Weather remains last.
