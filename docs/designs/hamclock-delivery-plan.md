...
- Next #288 slice: `.worktrees/hamclock-spot-age-source`, branch `feat/hamclock-spot-age-source`, based on #452. Own the stored-feed 15/30/60-minute API contract in `api/_lib/spotStore.ts`, its tests, `api/_lib/handlers/spots.ts`, route tests and a small shared parser/contract if needed. Separate freshness from history cutoff, keep stale/unavailable evidence and bounded requests, then wire client hooks and Settings age selection in the following slice. Global 6h/24h sampled history remains unsupported by this contract; personal PSK map integration follows separately. No collector/database/3D/model internals are claimed. Density fixture browser/server and its verified stale record are cleaned up.
