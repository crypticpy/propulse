...
Totals: 16 delivered, 3 partial, 35 not started.
...
- **Do not touch:** `wall/reports/*`, `HamClockWallHeader.tsx`, `kioskStore.ts` (page-id pins come with B7)
...
- **In scope:**
  ...
  - Map tab: style rows (swatch, name, behaviour line, selected dot, keyboard hint) over `mapStyle`, tile provider and `nightLights`, plus `LAYER_PRESETS` rows
...
- **Files to touch:**
  ...
  - `src/stores/mapStore.ts` (read only unless a setter is missing)
...
- **PR:** ≤ 15 files, title `feat(hamclock): layer registry, layers tab, map style chooser [B6: HW-21, HW-39]`, branch `feat/hamclock-b6-<slug>`
- **Do not:** add flyouts or side panels; open anything on hover; put a scroll region inside a tile, rail, report or settings tab; hard-code colours, hex values or Tailwind colour classes; relax bundle budgets, lint rules or thresholds; force push. Work in a worktree branched from `origin/main`.
...
- **Files to touch:**
  ...
  - `src/stores/kioskStore.ts`, `src/lib/kiosk/applySceneToMap.ts`
...
- **Covers:** HW-17, HW-29, HW-30, HW-31
...
- **PR:** ≤ 15 files, title `feat(hamclock): trend charts, report pin, best band table, forecast horizons [B9: HW-17, HW-29, HW-30, HW-31]`, branch `feat/hamclock-b9-<slug>`
- **Do not:** add flyouts or side panels; open anything on hover; put a scroll region inside a tile, rail, report or settings tab; hard-code colours, hex values or Tailwind colour classes; relax bundle budgets, lint rules or thresholds; force push. Work in a worktree branched from `origin/main`.
