...
| HW-11 | Honest empty states and freshness in reports                                                | Delivered   | PR #239                               | every report footer now uses the shared `DATA: source · UPDATED hh:mm UTC · age` contract via `reportFooter()`; `LocalWeatherData.observedAt` derives the true UTC instant from Open-Meteo's `utc_offset_seconds`; `WeatherReport`'s condition text moved out of the `updated` slot into the verdict/facts               |
...
| HW-17 | Forecast horizon                                                                            | Delivered   | PR #239                               | `ForecastReport` reads `FUTURECAST_HORIZONS_HOURS`/`propagationFutureCastHorizonIsActivated`; when a horizon is signed-off active, its matching matrix hour column is marked (`hcr-matrix-head--model`, sr-only table twin) and a MODEL fact is added — no new data feed, an overlay on the existing 24 h physics matrix |
...
| HW-29 | Trend charts in reports, chart components read theme tokens                                 | Delivered   | PR #239                               | `SolarMiniChart`/`SolarSeriesChart` now read `--hcr-chart-*` (falling back to their prior literal hex, so `/solar` is unchanged); every report grew one titled trend chart (`METRIC — RANGE · SRC`); `MetricCard` excluded — it has no sparkline                                                                         |
| HW-30 | Report pin                                                                                  | Delivered   | PR #239                               | spelled-out PIN/UNPIN control (≥44px, `aria-pressed`) in `WallReport`; session-only, non-persisted store; `HamClockPinnedReportHost` mounted once in `HamClockWallHeader` so a pin survives page/scene navigation; closes only on UNPIN/Escape                                                                           |
| HW-31 | Best Band Now ranked table report                                                           | Delivered   | PR #239                               | new `wall/reports/BestBandReport.tsx`: full ladder ranked per §10 tie-break, ≥44px row buttons set band focus/spot filters, sr-only table twin, separate surprise-activity section; `BestBandTile` now opens it in place of `BandVerdictDetailsDialog`                                                                   |
...
| HW-36 | Widget config contract and `hamclockWidgetConfigStore` | Delivered | PR #480 | Reuses B0 schema validation; shared registered-panel dialog |
| HW-37 | News feeds config dialog (first configurable widget) | Delivered | PR #479, #480 | Verified URL add, source state/refresh, polling and age controls, paginated rows |
...
| HW-47 | Monitored regions and RIM scores as a report                                                | Delivered   | PR #1089                              | `RimReport` REGIONS tab; selecting a region re-scopes lightning, TEC and NVIS. HW-47 moved here from B16.                                                                                                                                                                                                              |
...
| HW-56 | `EngineComparisonStrip`: physics / nowcast / observed on every model-backed report          | Delivered   | PR #246                               | section 26.1; `EngineComparisonStrip.tsx`, pure `compareEngines()` in `src/lib/hamclock/engineComparison.ts`; on the MUF and Best band reports                                                                                                                                                                           |
| HW-57 | MUF report: ionosphere facts, hop table, usable-window chart                                | Delivered   | PR #246                               | section 26.2; `MufReport.tsx`, `useMUFHourlySeries`; PATH / HOPS tabs, shaded FOT–LUF usable window, hop rows flash the map                                                                                                                                                                                              |
| HW-58 | Reliability report: SNR, confidence, station inputs, three-engine chart | Partial | PR #570 | Presentation/current evidence and stable pinned selection delivered; historical model/observed series, hop count and physical review remain. |
| HW-59 | Propagation forecast report: two UTC days, matrix and FutureCast horizons | Partial | PR #571 | Physics presentation and gated horizons delivered; active=[] until a horizon-aware scorer/response contract exists; physical review remains. |
| HW-60 | Solar report: SFI, SSN, flux forecast, cycle 25 chart                                       | Delivered   | PR #247                               | section 26.5; `SolarReport.tsx` rewritten with NOW / CYCLE tabs, flux trend with the 27-day outlook tail                                                                                                                                                                                                                 |
| HW-61 | X-ray and flares report: flux curve with B/C/M/X, latest flare, D-RAP, probabilities        | Delivered   | PR #247                               | section 26.6; `XrayReport.tsx`, FLUX (log axis, B/C/M/X rules, last flare marker) / ABSORPTION (D-RAP) / PROBABILITIES tabs                                                                                                                                                                                              |
| HW-62 | Solar wind and geomagnetic report: Bz / speed / density, Kp, Dst, aurora, CMEs, protons     | Delivered   | PR #247                               | section 26.7; `SolarWindReport.tsx`, WIND / GEOMAGNETIC / EVENTS tabs, Bz zero rule, aurora map link                                                                                                                                                                                                                     |
| HW-63 | Sun report: rise / noon / set, twilights, elevation curve, day-length trend                 | Delivered   | PR #243                               | section 26.8; `SunReport.tsx`, `src/lib/hamclock/sunCurve.ts`; elevation curve with shaded twilights, polar day/night states                                                                                                                                                                                             |
| HW-64 | Grey line report: per-band 160/80/40 tiers, windows, target overlap                         | Delivered   | PR #243                               | section 26.9; `GreyLineReport.tsx`, `getGreylineIntensityCurve` / `getMutualGreylineWindow` in `greyline.ts`; tiers, mutual overlap with the DX target                                                                                                                                                                   |
| HW-65 | EME computation module `src/lib/utils/eme.ts`                                               | Delivered   | PR #253                               | EME module: eme.ts path loss, degradation, Doppler, sky noise, mutual window                                                                                                                                                                                                                                             |
| HW-66 | Moon and EME report                                                                         | Delivered   | PR #253                               | Moon report: MOON/EME tabs, band selector, sub-lunar point                                                                                                                                                                                                                                                               |
...
| HW-69 | Radio Impact Model tile                                                                     | Delivered   | PR #1089                              | `RimTile` on the Weather page over `computeRIM` / `useRIM`; section 26.13; unavailable sub-scores dropped from the composite                                                                                                                                                                                              |
...
| HW-71 | Recent contacts report: log statistics and 30-day chart | Partial | PR #407, #550, #561 | Report, exact logged-frequency tuning and metadata transport/repair delivered; authenticated two-device and physical review pending. |
...
Totals: 49 delivered, 7 partial, 17 not started.
