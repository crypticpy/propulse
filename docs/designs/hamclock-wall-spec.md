...
| HW-11 | Honest empty states and freshness in reports                                                | Delivered   | PR #TBD                               | every report footer now uses the shared `DATA: source · UPDATED hh:mm UTC · age` contract via `reportFooter()`; `LocalWeatherData.observedAt` derives the true UTC instant from Open-Meteo's `utc_offset_seconds`; `WeatherReport`'s condition text moved out of the `updated` slot into the verdict/facts               |
...
| HW-29 | Trend charts in reports, chart components read theme tokens                                 | Delivered   | PR #TBD                               | `SolarMiniChart`/`SolarSeriesChart` now read `--hcr-chart-*` (falling back to their prior literal hex, so `/solar` is unchanged); every report grew one titled trend chart (`METRIC — RANGE · SRC`); `MetricCard` excluded — it has no sparkline                                                                         |
| HW-30 | Report pin                                                                                  | Delivered   | PR #TBD                               | spelled-out PIN/UNPIN control (≥44px, `aria-pressed`) in `WallReport`; session-only, non-persisted store; `HamClockPinnedReportHost` mounted once in `HamClockWallHeader` so a pin survives page/scene navigation; closes only on UNPIN/Escape                                                                           |
| HW-31 | Best Band Now ranked table report                                                           | Delivered   | PR #TBD                               | new `wall/reports/BestBandReport.tsx`: full ladder ranked per §10 tie-break, ≥44px row buttons set band focus/spot filters, sr-only table twin, separate surprise-activity section; `BestBandTile` now opens it in place of `BandVerdictDetailsDialog`                                                                   |
...
Totals: 33 delivered, 2 partial, 20 not started.
