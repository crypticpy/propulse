...
## W03 executable revision and layout decisions

W03 stores no second mutable draft graph: `draftRevisionId` names the latest immutable candidate. Pure initial/edit/clone/restore proposals return the expected prior head alongside their new records. W04 must check that head and unused IDs atomically when committing. A timestamp records an instant but cannot determine ordering across devices; lineage and compare-and-swap establish the transition. Explicit parent/source provenance is acyclic, and restore appends rather than rewriting or rewinding history. Historical imports may omit the new transition discriminator; new services always provide it.

Cloning remaps setup-local connection/run/route IDs together while retaining shared physical equipment/port/internal-path identity. It copies historical specifications, evidence and location, plus unchanged source setup recovery envelopes; it does not refresh from today's catalog or duplicate inventory. Explicit `settings.bandId` records declared band intent without inferring it from frequency.

Diagram, list and rack layouts reference the same revision. Optional item order is a complete permutation of revision membership; partial coordinates represent unplaced equipment. Label/port/grid/snap view preferences are presentation-only. Global visual-comfort settings remain outside these records. Invalid graphs retain private original JSON with diagnostics, while unsupported valid documentation remains explicitly unsupported. These foundations do not satisfy the later persistence, engine, operating, accessibility or feature-cutover gates.

## Review follow-up: mating interfaces and modeled-result honesty

A cable run's base identity and length do not bind its connectors to equipment ports. Optional per-connection `connectorInterface` records either a direct mating interface or the base cable's explicit from/to ports and internal path. These physical IDs remain shared when setup-local graph IDs are cloned. Missing bindings remain unknown, not an invented cable termination. A fully direct run cannot also claim a physical base cable. A bound termination cannot simultaneously serve as an ordinary connection endpoint; hidden internal branches must not become a supported linear cable calculation.

Compatibility must use the actual modeled band frequency and stage power, not only the requested radio input. `port.maxPower` is a declared maximum rating; `port.rfPower` is an RF reading and cannot stand in for that maximum. Unsupported components, ignored known losses/gains and engine fallback inputs must withhold affected modeled results. Caller or pinned mode is validated explicitly; unknown modes cannot silently select the engine's default. Manufacturer/tested attribution must follow actual evidence, not merely a catalog field namespace.

The publication policy rejects contradictory states for one grant identity and contradictory section overrides for fixed module kinds. This pure policy still requires server assembly bound to the exact owner/publication/version/reviewed revision; it does not replace W05 API/RLS/media/cache verification.
