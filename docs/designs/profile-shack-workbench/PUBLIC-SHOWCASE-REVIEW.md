...
The existing `parsePublicEquipmentSummary` and `usePublicEquipmentImage(ownerUserId, photoId)` remain the data boundary. Names, station text, node labels/order (including duplicates), antenna type, power, ERP and both photo references are retained. Zero watts remains visible; missing, negative or non-finite values read “Not shared.” Positive values below one watt use milliwatts with significant precision, including values below one milliwatt. ERP is explicitly estimated, and the summary does not assert complete wiring or live hardware state. Photo failures fall back to a generic equipment drawing.
...
- Thirteen focused component tests: invalid summaries, metadata/node preservation, owner-bound photo lookup, zero/unknown and small-positive values, no interactive controls and photo failure/replacement.
