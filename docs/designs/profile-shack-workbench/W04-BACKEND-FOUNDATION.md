...
Every execution and cleanup verifies the exact container ID and ownership labels. Cleanup removes that container on success, failure or handled interruption; if creation fails before writing a CID file, cleanup inspects only that run’s exact generated UUID name and verifies its ID, name, ownership labels, pinned image and isolation before removing it by ID. Only an explicit missing-container response establishes absence; an inspection failure retains the ownership diagnostic (and CID file when available) for diagnosis. Existing containers and databases are outside this runner's scope.
