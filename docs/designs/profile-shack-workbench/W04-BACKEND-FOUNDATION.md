...
Every execution and cleanup verifies the exact container ID and ownership labels. On success, failure or handled interruption, cleanup attempts to remove only that verified container; if creation fails before writing a CID file, cleanup inspects only that run’s exact generated UUID name and verifies its ID, name, ownership labels, pinned image and isolation before removing it by ID. After an ambiguous creation failure, even an exact-name missing-container response cannot establish completed cleanup: the daemon may create the container after that lookup. The runner reports uncertain cleanup and retains `ownership.json` with the generated run ID, exact name, pinned image and Docker context for follow-up; it does not treat bounded polling as proof of permanent absence. Other inspection failures also retain the ownership diagnostic (and CID file when available). Existing containers and databases are outside this runner's scope.

Optional repeated `--migration` and `--fixture` flags select individual workbench SQL files inside the checkout, in the supplied order. Basenames must use `station_workbench` or `station-workbench`, optionally preceded by a 14-digit timestamp and `_` or `-`; an optional descriptive suffix must also begin with `_` or `-`. Examples are `20260905012345_station_workbench_schema.sql` and `station-workbench-fixture.sql`. Other namespaces, including existing StationCast migrations, are rejected before Docker starts. The runner never discovers or replays the repository's complete migration directory. Selected files are detached before Docker starts and checked against the harness ownership marker. This intentionally restricted SQL subset rejects every backslash and NUL character, including those inside strings or comments, so a file cannot issue psql control commands; use SQL expressions such as `chr(92)` when a literal backslash is required. Each selected file’s remaining transaction is explicitly committed before the ownership check and completion sentinel, so an accidentally unclosed transaction cannot silently roll back after a PASS. Fixtures must explicitly `ROLLBACK` their transient changes. Every SQL invocation requires successful exit and a fresh exact completion sentinel after the same-session ownership check.
