...
6. Reject context-independent transition/lineage violations before CAS. When a stale proposal lacks a complete historical validation context, preserve its original envelope and both bases with an explicit quarantined classification; it is not an aggregate-validated alternative or sendable work. Return conflict only after its recovery transaction completes. Certifying a valid alternative requires complete historical context or a new reviewed replacement against current state; no candidate head/tombstone becomes canonical merely because it was retained.
