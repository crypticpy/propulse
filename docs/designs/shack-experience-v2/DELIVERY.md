...
| E — Integrated evidence | Release gate [#706](https://github.com/crypticpy/propulse/issues/706) owns the cross-cutting SX2:Q01-Q05 checks and is blocked by every implementation package plus #184/#185; the existing #185/#194/#195 gates and the media/sale scenarios feed it. Migration, access, rollback, bundle and real-operator checks land as evidence on #706. | Evidence-based cutover with all retained capabilities accounted for; phase E is not complete until #706 closes |
...
The current `usePublicEquipmentImage` constructs owner-bound public URLs in `equipment-images`, and existing radio, antenna and accessory records can hold up to five gallery IDs — `AccessoryBase.galleryImageIds` applies to every `AccessoryCategory` (amplifiers, tuners, filters, switches, power supplies and the rest), not to amplifiers alone, so the audit and the migration fixtures must cover all of them or #277/M03 will drop gallery references already stored on non-amplifier accessories. This source audit does **not** verify deployed bucket policy or establish an album system. Audit deployed access and preserve old references before migration. Do not put new private originals into a bucket assumed private merely because its UI says so.
...
Canvas refinement remains in #184 and accessible interaction in #185; real-operator review and cutover remain #194/#195. The cross-cutting quality and delivery gates SX2:Q01-Q05 belong to release gate #706, which is blocked by #184, #185, #277, #278, #279, #280, #281 and #283 — no individual package discharges them, and closing every package in this table still leaves #706 open until its evidence is recorded. New packages are children of #276, which extends #173. Closed contract prerequisites remain recorded because implementation depends on their delivered contracts. Native GitHub dependencies and Project 3 track readiness; this table and coverage register define requirement ownership.
