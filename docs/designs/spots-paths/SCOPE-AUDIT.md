...
| ProfilePopover listener shortcut | Direct mapStore.setState merges listener layers/activePreset, then settingsStore.updateUIInteraction changes spot colors/style | SP-03/SP-08 replace with the bound runtime preset command; SP-09 wires the existing popover. It must not retain a singleton shortcut around scoped preset application. |
