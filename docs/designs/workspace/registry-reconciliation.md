...
| D2 `contests` | In `HOME_ONLY_IDS` with `densities: ["glance"]` only. Also a wall tile (`ContestsTile`). Wall title `"Contests"`; registry title `"Contest details"`. Home layout uses the registry title. | Dual-canvas, not home-only. Add `"wall"` to `densities`, add `"contests"` to `WALL_SEEDED_IDS`, remove it from `HOME_ONLY_IDS`. Keep glance for Home. Pick one title in the code PR; wall `"Contests"` vs Home `"Contest details"` is an owner call (this paper does not pick). |
