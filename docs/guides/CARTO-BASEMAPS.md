...
[CARTO's basemap terms, section 9.c](https://carto.com/legal/basemap-terms/), allow direct requests by application users and prohibit server-side proxying/caching. There is no CARTO tile endpoint, server cache or credential forwarding in this integration. End-user caching must not exceed 30 days. The service worker's `tiles-carto` rule (`src/lib/tiles/tileRuntimeCaching.ts`) is bounded to one day, matching the CARTO provider's application cache TTL, and stores only tiles the reader actually viewed. Do not raise that retention toward the 30-day ceiling, and do not introduce bulk tile downloads: there is no CARTO prefetch, warm-up or offline pack, and adding one would breach these terms. Clear retained CARTO data if the app ceases using the service.
