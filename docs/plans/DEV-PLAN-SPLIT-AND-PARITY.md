...
> **Status (2026-08-29): E5 implemented on `feat/split-parity`.**
>
> - **E5.1 done.** 29 routes extracted into 13 handler modules under `api/_lib/handlers/` (4 commits, one per domain group); cloud-bound routes untouched. `api/_lib/portableRoutes.ts` is the new single manifest (46 routes incl. all 20 solar; the five auth-gated model-service proxies `/api/propagation/{path,surface,models,health,capabilities}` are deliberately excluded — they verifyAuth and spend the paid inference token). Deviations: spots wrappers keep their `{ fetch }` export shape (test contract); same-named private helpers renamed where files merged; `api/satellites/tle` was originally classified cloud-bound but has a full Celestrak/AMSAT fallback when Supabase env is absent, so it joined the manifest un-extracted (imported whole, like the solar wrappers).
> - **E5.2 done.** Bridge build gains `scripts/bundle-bridge-api.mjs` (esbuild → `bridge/dist/portableRoutes.mjs`, ~780 kB); `bridge/src/apiMount.ts` adapts Node http ⇄ Web Request/Response and answers unknown `/api/*` with an honest 404. LAN exposure is opt-in via **`BRIDGE_STATIC_HOST`** (new env; the rig-control WebSocket stays hard-locked to localhost — deliberate deviation from "set BRIDGE_HOST=0.0.0.0"). mDNS (`bonjour-service`, `propulse.local`) starts only when LAN-bound. Settings sync: `bridge/src/settingsSync.ts` persists the frontend's existing `SettingsBackup` blob at `$BRIDGE_DATA_DIR/lan-settings.json` (`/api/bridge/settings` GET/PUT, 1 MB cap, atomic write); clients auto-pull every 30 s (`useLanSettingsSync`), publishing is an explicit button in Settings → Data (deviation from "poll/subscribe": push is manual by design so one device's transient tweak can't silently overwrite the shack).
> - **E5.3 done.** `connectivity: cloud|lan|offline` + `setConnectivity` in `dataSourceStatusStore`; `useConnectivityTier` (navigator.onLine + bridge-origin check: port 3173 or `.local` host); `ConnectivityBadge` LAN pill in both headers. Offline visual stays with the pre-existing `OfflineIndicator` banner (no duplicate badge).
> - **E5.4 done.** Both guides committed; SELF-HOSTING.md updated to the real E5.2 mechanics.
> - Verified live: bridge served a no-Supabase build at `127.0.0.1:3173` — LAN badge rendered, no login gate, k-index/aurora/scp/TLE 200 through the mount, cloud-only route 404 with honest message, publish→persist→fresh-device-auto-pull round trip. **Not yet tested on a real LAN/mDNS** (needs a second device; owner follow-up).
> - **Gate ran 2026-08-29** (`/freview` completeness + principal, plus the commit security scan). All blockers fixed same day: request-body cap (1 MB, 413) in the bridge API mount; Host-header allowlist (`bridge/src/hostGuard.ts`) as DNS-rebinding defense; rate-limit identity pinned to the socket address (client-supplied `x-forwarded-for` ignored); auth-gated model-service proxies dropped from the manifest; docs build command corrected (`npm run build`, not bare `npx tsc`); esbuild declared as a root devDependency; unique temp-file suffix + 413-before-destroy in settings sync. Accepted as documented trade-offs: settings blob has no auth beyond the Host guard + LAN trust model; LAN tier detection is heuristic (port 3173 / `.local`).
> - ~~Open question for owner~~ **Resolved 2026-08-29 (owner):** wall/LAN devices run env-unset builds (the self-host path) — the invite-beta login gate stays fully intact; no origin-based bypass will be built.
...
> **Status (2026-08-29): E6 + E7 implemented on `feat/split-parity`.**
>
> - **Backend done.** 7 new portable handlers + the JTWC extension on `atmosSpace.ts`; manifest now 53 routes. Every route smoke-tested through the real esbuild bridge bundle against live upstreams (METAR KAUS/KSAT, volcanoes n=5 with CAP coordinate enrichment, tides Bob Hall Pier, UV 7.9, AQI `configuration_missing` degrade, DXpeditions n=46, RSS ok n=15, tropical `activeStorms`/`jtwc`/`jtwcAvailable`). New generic `portableDevApi()` Vite middleware serves the whole manifest in dev (registered after the inline proxies so env-fallback mocks win) — future manifest routes get dev parity for free.
> - **G14 (user RSS) is the SSRF boundary:** scheme http(s) only, no ports/credentials/IP literals (incl. decimal/hex), no localhost/single-label/`.local`/`.localhost`/`.internal`/`.home.arpa`, per-hop redirect re-validation (max 3), 1 MB streamed cap; item links pass the same gate; entities decoded *before* tag-strip so `&lt;script&gt;` can't materialize. Residual DNS-rebind risk documented (no portable resolver API).
> - **Frontend done.** 7 new dashboard cards + `QthScopeCard` (G18: canvas range rings, sweep, age-faded strikes, opt-in proximity ping) in two new Home rows + mobile subset (Environment, DXpeditions); `feedStore` (persisted, max 5 feeds, ARRL default). `useTropicalCyclones` refactored to one fetch for both NHC and JTWC.
> - **E7 done.** `useLaunchWall` (`getScreenDetails()` feature-detect, round-robin scene assignment, positioned popups per screen, cascade fallback, popup-block count + hint) behind a Launch Wall section on `/kiosk`.
> - **Deviations:** ADXO fetched from `ng3k.com/Misc/adxo.html` (the `www` path is a 75-byte meta-refresh stub fetch can't follow — fixture-passing parsers still need a live-URL smoke); TidesCard renders hilo-only for subordinate NOAA stations (no interval curve; sparkline hidden, >150 km distance flagged amber); AQI ships `configuration_missing` until owner obtains `AIRNOW_API_KEY`/`WAQI_TOKEN`.
> - Live-verified on a no-Supabase dev build: all 8 new surfaces render real data (Denver QTH: Puerto Peñasco tides 1216 km flagged not-local). Console errors present are environmental (bridge WS down, pre-existing TLE dev-proxy drift), not E6/E7.
> - **Gate ran 2026-08-29** (`/freview` completeness + principal). One blocker fixed same day: trailing-dot FQDNs (`localhost.`, `metadata.google.internal.`) bypassed the RSS SSRF host checks — hostname now strips trailing dots before all checks (regression tests added). Minors fixed: out-of-range numeric entities crashed `decodeEntities` (whole feed read "unreachable"); `bboxAround` now slides inward at the antimeridian instead of producing a 400; METAR/volcano upstream reads stream-capped instead of buffer-then-check; NewsFeedCard surfaces an SSRF-gate 400 instead of a blank ok-state; JTWC systems got their render surface (info alerts via `weatherAlertEngine`, same pattern as NHC); `AIRNOW_API_KEY`/`WAQI_TOKEN` added to `.env.example`. Accepted as noted trade-offs: `parseLatLon` duplicated per-handler (private validators, consolidation deferred); `station`/`ids` query params app-unreachable but deliberate self-host API surface.
...
## 8. Punchlist (scheduled 2026-08-29 — owner approved P1+P2 into the post-M3 work block)
...
- **M4** = Phase-3 forecast-engine work — separate dev plan when M3 closes (eval harness first; the dead WSPR ingest stays dead; any historical store is designed fresh under an approved budget cap). **Authorized 2026-08-29 (owner): plan + execution approved; historical-store budget cap set at ~$25/mo.** Plan: `DEV-PLAN-FORECAST-ENGINE.md`.
- **Post-M3 work block (owner-approved 2026-08-29):** G19 monthly Blue Marble assets (web-size, committed — PR #52) + punchlist P1/P2 + M4 kickoff.
