...
> - Open question for owner: a bridge-served device with a Supabase-configured build still hits the invite-beta login gate. Documented answer is building with `VITE_SUPABASE_URL` unset; if wall devices should instead bypass the gate by origin, that's a product call (weakens the client-side beta gate).
