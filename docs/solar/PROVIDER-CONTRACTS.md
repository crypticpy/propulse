...
| `swpc-solar-wind-plasma` `/api/solar/wind-plasma` | NOAA SWPC [RTSW one-minute plasma](https://services.swpc.noaa.gov/json/rtsw/rtsw_wind_1m.json) | `time_tag`; ascending latest 24-hour window, observedAt equals newest row | Speed km/s, density cm⁻³, temperature K; independently nullable, speed or density required | 2,500 rows, 4 MB upstream, 320 kB output; no usable speed/density fails | Current reading and wall-report history (expanded in #244) |
