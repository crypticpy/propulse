...
if [[ "$(/usr/bin/arch)" != "arm64" ]]; then
  print -u2 "Prospective collector migration commands are M5-only"
  exit 1
fi
...
if [[ "${MODE}" != "--dry-run" && "${MODE}" != "--apply" ]]; then
  print -u2 "Usage: $0 [--dry-run | --apply --acknowledge-collector-migration]"
  exit 1
fi
if [[ "${MODE}" == "--apply" && "${ACKNOWLEDGEMENT}" != "--acknowledge-collector-migration" ]]; then
  print -u2 "Apply requires --acknowledge-collector-migration"
  exit 1
fi
