"""Build the path-hour training table from the slim spot Parquet.

Steps:
 1. Backfill missing grid fields (RBN has none) via a callsign -> field map
    built from grid-bearing spots (a callsign maps only if one field accounts
    for >= 80% of its sightings — filters rovers/portables).
 2. Aggregate positives: (hour_utc, band, tx_field, rx_field) cells.
 3. Candidate universe: pairs with >= MIN_PAIR_SPOTS spots *in the train
    window* (avoids holdout leakage in universe selection), cross-joined with
    hours where the TX field transmitted anything and the RX field heard
    anything (any band) — so negatives mean "both ends were on the air, the
    path/band produced nothing".
 4. Join hourly solar, compute geometry features (distance, bearing, solar
    elevation at endpoints and path midpoint).

Output: ml/data/processed/train_cells.parquet
"""
...
OUT = "ml/data/processed/train_cells_v3.parquet"
CENTROIDS = "ml/data/processed/field_centroids.parquet"

TRAIN_END = "2026-03-23"  # universe selection uses data before this only
MIN_PAIR_SPOTS = 300      # pair must have this many spots in train window
...
con.execute(
    ...
    SELECT
        s.hour_utc, s.band, s.mode, s.source, s.snr,
    ...
    WHERE coalesce(s.tx_field, ct.field) IS NOT NULL
      AND coalesce(s.rx_field, cr.field) IS NOT NULL
    """
)
stats = con.execute(
    "SELECT count(*), count(*) FILTER (source='rbn'), min(hour_utc), max(hour_utc) FROM spots"
).fetchone()
log(f"usable path spots: {stats[0]:,} (rbn recovered: {stats[1]:,}) | {stats[2]} -> {stats[3]}")
...
log("per-band region activity tables built")

# adjacency map for neighbor-activity features (lon wraps A<->R, lat clamps)
adj_rows = []
for lo in range(18):
    for la in range(18):
...
# neighbor-field activity at t-1: same band, tx side and rx side
con.execute(
    """
...
log("neighbor activity tables built")
...
# solar dynamics: storm onset / flare history (rows are hourly, near-complete)
con.execute(
    """
...
# All strictly <= t-1 (deployable against the live spot feed, no leakage).
# RANGE frames are time-based, so zero-spot hours missing from the grid are
# handled correctly.
con.execute(
    """
    ...
    LEFT JOIN pos p1 ON p1.hour_utc = c.hour_utc - INTERVAL 1 HOUR
        AND p1.band = c.band AND p1.tx_field = c.tx_field AND p1.rx_field = c.rx_field
    LEFT JOIN pos p24 ON p24.hour_utc = c.hour_utc - INTERVAL 24 HOUR
        AND p24.band = c.band AND p24.tx_field = c.tx_field AND p24.rx_field = c.rx_field
    LEFT JOIN pos pr ON pr.hour_utc = c.hour_utc - INTERVAL 1 HOUR
        AND pr.band = c.band AND pr.tx_field = c.rx_field AND pr.rx_field = c.tx_field
    ...
    WINDOW wpath AS (PARTITION BY c.tx_field, c.rx_field, c.band ORDER BY c.hour_utc)
    """
)
log("lag features attached")

# Keep the table tractable: downsample TRAIN-window negatives (holdout stays
# complete so evaluation metrics are on the true base rate).
...
# activity-weighted centroids override geometric centers where known
try:
...
# solar elevation (subsolar point from timestamp; NOAA low-precision formulas)
ts = pd.to_datetime(df["hour_utc"], utc=True)
doy = ts.dt.dayofyear.to_numpy()
frac_hour = ts.dt.hour.to_numpy() + 0.5  # mid-hour
...
# ---- path darkness profile: sample the great circle at 7 points (slerp)
...
for tfrac in np.linspace(0.0, 1.0, 7):
    ...
    p = np.where(norm > 1e-9, p / np.where(norm > 1e-9, norm, 1.0), v1)  # degenerate: same field
...
df["min_abs_elev_ends"] = np.minimum(np.abs(elevs[:, 0]), np.abs(elevs[:, -1]))  # gray-line
df["path_min_elev"] = elevs.min(axis=1)
df["path_max_elev"] = elevs.max(axis=1)

# ---- crude MUF proxy (secant law over an E/F-layer ionization estimate)
...
half_hop = df["dist_km"].to_numpy() / n_hops / (2 * R_E)  # radians
...
# ---- calendar flags: weekends + major HF contest weekends in window
df["is_weekend"] = (ts.dt.dayofweek >= 5).astype(int)
CONTEST_DATES = {  # CQ WPX RTTY, ARRL DX CW, ARRL DX SSB, CQ WPX SSB (2026)
