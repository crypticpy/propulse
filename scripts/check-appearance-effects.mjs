...
if (
  ...
  throw new Error("Pass a managed local origin such as http://127.0.0.1:5173");
}
...
if (!identityResponse.ok)
  throw new Error("Managed server identity is unavailable.");
...
if (
  ...
  throw new Error(
    "Use this checkout's managed local-profile server; check dev:session status.",
  );
}
...
try {
  ...
  await expect(
    section.getByRole("radio", { name: "Subtle", exact: true }),
  ).toBeChecked();
  await choose(page, section, "Full");
  await section
    .getByRole("switch", { name: "Decorative glow", exact: true })
    .uncheck();
  await choose(page, section, "Off");
  await page.reload();
  await expect(
    section.getByRole("radio", { name: "Off", exact: true }),
  ).toBeChecked();
  await expect(
    section.getByRole("switch", { name: "Decorative glow", exact: true }),
  ).not.toBeChecked();
  ...
  const other = second.getByRole("region", {
    name: "Decorative effects",
    exact: true,
  });
  await choose(second, other, "Full");
  await expect(
    section.getByRole("radio", { name: "Full", exact: true }),
  ).toBeChecked();
  record("Cross-tab updates apply to the original page");

  await page.emulateMedia({ reducedMotion: "reduce" });
  await expect(
    section.getByText("Reduced motion is active", { exact: true }),
  ).toBeVisible();
  ...
  const glow = section.getByRole("switch", {
  ...
  const checkWidth = async () => {
    ...
    assert.ok(
      bounds.width > 0 && bounds.scroll <= bounds.width + 1,
  ...
  for (const [name, id] of [
    ...
    const violations = await section.evaluate(async (element) => {
      ...
      return audit.violations.map(({ id: rule, nodes }) => ({
        id: rule,
        nodes: nodes.length,
      }));
    });
  ...
  record(
    "Four themes: new controls pass scoped axe WCAG 2A/AA/2.1AA and width checks",
  );
  ...
  await page
    .getByRole("heading", { name: "Appearance", exact: true, level: 1 })
    .waitFor();
  await section.waitFor();
  await page.evaluate(() => {
  ...
  await gridPage
    .getByRole("heading", { name: "Real MouseTilt grid fixture" })
    .waitFor();
  ...
  for (const level of ["subtle", "off", "full", "subtle"]) {
    ...
    for (let index = 0; index < articles.length; index++) {
      assert.equal(
        await articles[index].evaluate(
          (article, position) =>
            article === document.querySelectorAll("[data-card]")[position],
          index,
        ),
        true,
      );
    ...
    await expect
      .poll(
        async () =>
          gridPage.locator("[data-card-grid]").evaluate((grid) => {
            ...
            return [...grid.querySelectorAll("[data-card]")].every((card) => {
              ...
              return (
                ...
                Math.abs(box.height - row.height) <= 1
              );
            });
          }),
        { message: `Cards must stretch to the grid row in ${level}` },
      )
      .toBe(true);
    await gridPage
      .getByRole("button", {
        name: `Short card clicks: ${clicks}`,
        exact: true,
      })
      .click();
    clicks++;
    await expect(
      gridPage.getByRole("button", {
        name: `Short card clicks: ${clicks}`,
        exact: true,
      }),
    ).toBeVisible();
  ...
  record(
    "Real MouseTilt cards retain equal row height, click state, child identity and focus across Subtle/Off/Full",
  );
