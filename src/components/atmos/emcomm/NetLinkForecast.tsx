...
export function NetLinkForecast() {
  ...
  if (!activePlan) {
    return (
      <div className="text-center py-2">
        <p className="text-xs text-su-muted">Select a frequency plan</p>
      </div>
    );
  }
  ...
  if (!forecast || forecast.length === 0) {
    return (
      <div className="text-center py-2">
        <p className="text-xs text-su-muted">No frequencies in plan</p>
      </div>
    );
  }
      ...
      {forecast.map(({ entry, hours }, rowIdx) => (
        ...
        >
          ...
          <div className="w-16 shrink-0 text-right pr-2">
            <span className="text-xs font-mono text-su-muted truncate block">
      ...
      <div className="flex items-center gap-3 pt-1">
        {(["good", "marginal", "poor"] as const).map((q) => (
          <div key={q} className="flex items-center gap-1">
            ...
            <span className="text-xs font-mono text-su-muted capitalize">
              {q}
            </span>
          </div>
        ))}
