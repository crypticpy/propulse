...
function AlertMatchSection({ match }: { match: RepeaterAlertMatch }) {
  ...
  return (
    <div className="border border-su-line/40 rounded p-2 space-y-1.5">
      {/* Alert header */}
      ...
      <div className="text-[10px] text-su-muted">
        {match.repeaters.length === 0 ? (
          ...
          <button
            ...
            className="flex items-center gap-1 hover:text-su-text transition-colors"
          >
            <span
      ...
      {expanded && match.repeaters.length > 0 && (
        <div className="space-y-1 pl-2 border-l border-su-line/40">
          {match.repeaters.map((r) => (
            ...
            >
              ...
              <span className="text-su-muted shrink-0">{r.mode}</span>
              {r.tone && (
                <span className="text-su-muted shrink-0">{r.tone} Hz</span>
              )}
            </div>
          ))}
        </div>
