/**
 * Auth sub-text-xs guard — batch 44 (#808)
 *
 * Census on `origin/main`: remaining user-read auth components with sub-floor
 * `text-[Npx]` (N<12). This batch raises 1 user-read site in LoginPage.tsx.
 * Skips phone breakpoints.
 *
...
const FILES = ["src/components/auth/LoginPage.tsx"];
...
describe("sub-text-xs sizing stays at the floor in auth (#808 batch 44)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-44 auth files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
