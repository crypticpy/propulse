/**
 * Awards sub-text-xs guard (#808 batch 14)
 *
 * Census on `origin/main` at batch start: `src/components/awards/` holds 4
 * sub-floor `text-[Npx]` sites (N<12) across WazGrid, WasMap, and DxccGrid
 * slot tiles — user-read QSO counts and truncated names. This batch raises
 * them to `text-xs`. Shared status.ts / SlotDetailPanel (#1156) had no
 * remaining sub-floor sites; ContestContributions had none.
 *
...
const AWARDS_ROOT = resolve(REPO_ROOT, "src/components/awards");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = [
  "src/components/awards/WazGrid.tsx",
  "src/components/awards/WasMap.tsx",
  "src/components/awards/DxccGrid.tsx",
];
...
function walkAwardsSourceFiles(dir: string): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    if (statSync(abs).isDirectory()) {
      results.push(...walkAwardsSourceFiles(abs));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in awards (#808 batch 14)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed awards files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-14 files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no un-allowlisted sub-floor text-[Npx] anywhere under src/components/awards", () => {
    const violations: string[] = [];
    for (const abs of walkAwardsSourceFiles(AWARDS_ROOT)) {
