/**
 * Bands sub-text-xs guard (#808 batch 35b)
 *
 * Raises user-read sub-floor `text-[Npx]` (N<12) to `text-xs` in bands
 * components.
 *
...
const FILES = ["src/components/bands/BandPlanDisplay.tsx"];
...
describe("sub-text-xs sizing stays at the floor in bands (#808 batch 35b)", () => {
  it("has no sub-floor text-[Npx] or inline fontSize in the fixed bands files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-35b bands files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
});
