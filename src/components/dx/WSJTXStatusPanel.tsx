  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs
        ${cq ? "bg-plasma-orange/5" : "hover:bg-su-line/10"}
        transition-colors`}
    >
      {/* Time */}
      <span className="text-su-muted font-mono w-14 flex-shrink-0">
      ...
      <span
        className={`font-mono w-16 truncate flex-shrink-0 ${
          cq ? "text-plasma-orange font-bold" : "text-su-text/80"
        }`}
      ...
      {/* Grid */}
      <span className="text-su-muted font-mono w-10 flex-shrink-0">
        {decode.grid || ""}
      </span>

      {/* Message (truncated) */}
      <span className="text-su-muted truncate min-w-0">{decode.message}</span>
    </div>
  );
});
...
export const WSJTXStatusPanel = memo(function WSJTXStatusPanel({
  ...
  const indicatorColor = connected ? "bg-signal-green" : "bg-su-line";

  return (
    <div
      className={`bg-su-line/10 backdrop-blur-md border border-su-line/40 rounded-xl overflow-hidden ${className}`}
    >
      {/* Header */}
      <button
        type="button"
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-su-text uppercase tracking-wider">
            WSJT-X
          </span>
          {connected && status && (
      ...
      {!collapsed && (
        <div className="border-t border-su-line/40">
          {connected && status ? (
            <>
              {/* Status Bar */}
              <div className="flex items-center gap-3 px-3 py-2 bg-su-line/10 text-xs">
                {/* DX Call / Grid */}
                {status.dxCall && (
                  <div className="flex items-center gap-1">
                    <span className="text-su-muted">DX:</span>
                    <span className="text-su-text/80 font-mono font-medium">
                      {status.dxCall}
                    </span>
                    {status.dxGrid && (
                ...
                {/* Decode Rate */}
                <div className="flex items-center gap-1">
                  <span className="text-su-muted">Decodes:</span>
                  <span className="text-su-text/80 font-mono">
                    {decodeRate}/min
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1 ml-auto">
                  <span className="text-su-muted">Unique:</span>
                  <span className="text-su-text/80 font-mono">
              ...
              <div className="max-h-60 overflow-y-auto">
                {displayDecodes.length > 0 ? (
                  ...
                  <div className="px-3 py-4 text-center text-xs text-su-muted">
            ...
            <div className="px-3 py-6 text-center space-y-3">
              <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-su-line/10">
                <svg
              ...
              <div>
                <p className="text-sm text-su-muted">WSJT-X not connected</p>
