/**
 * Help sub-text-xs guard — batch 53 rest (#808)
 *
 * Census on `origin/main`: `HelpFeatureCompare.tsx` holds 2 sub-floor
 * `text-[Npx]` sites (N<12) — mobile Free/Pro tier labels. This batch
 * raises those user-read labels to `text-xs`.
 *
 * Skips HelpCallout, HelpTooltip, GettingStartedSection (#1189), Home,
 * Solar, and phone-only deferred labels.
 *
 * Sibling to `subTextSizeFloor.help.test.ts` — does not edit that file.
 */
...
const FILES = ["src/components/help/HelpFeatureCompare.tsx"];
...
describe("sub-text-xs sizing stays at the floor in help rest (#808 batch 53)", () => {
  it("has no sub-floor text-[Npx] or inline fontSize in the fixed help rest files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-53 help rest files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
});
