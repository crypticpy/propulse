...
function NearbyReports() {
  ...
  return <>{!guest && <ClusterReportsHost />}<Nearby locationOverride={location} publicOnly={guest} /></>;
}
export function HomeActivity({ now, isMobile }: { now: number; isMobile: boolean }) {
  ...
  return <section aria-label="Band activity" className="home-panel home-activity"><div className="home-panel-heading"><h2>On the bands now</h2><HomeStatus state={query.isPending ? "loading" : query.isError ? "error" : current ? "fresh" : query.data?.fetchedAt ? "stale" : "unavailable"} /></div>
    ...
    {selected && <div id="home-nearby-reports" className="home-detail"><p>Nearby reports use {location?.grid ?? "a location you choose"} and your saved range and time filters. This is a different population from the regional counts above.</p>{location ? <Suspense fallback={<p>Opening reports…</p>}><NearbyReports /></Suspense> : <p>Set your Home location to explore nearby reports.</p>}</div>}
  </section>;
}
