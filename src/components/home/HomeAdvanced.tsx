...
export function HomeAdvanced({ model, now, publicActivity }: { model: ReturnType<typeof useSolarModel>; now: number; publicActivity: boolean }) {
  ...
  return <section className="home-panel home-advanced su-section-panel" data-accent="info" aria-label="Advanced dashboard">
    <div aria-hidden="true" className="su-section-rule" />
    <SectionHeader className="su-widget-header" title="Current bands & the next 12 hours" summary="Reported activity now, and a modeled 12-hour path outlook you set the assumptions for." action={<Link to="/solar">Deeper solar detail ↗</Link>} />
