...
export function HomeAdvanced({ model, now, publicActivity }: { model: ReturnType<typeof useSolarModel>; now: number; publicActivity: boolean }) {
  ...
  const [targetGrid, setTargetGrid] = useState(guest ? "" : mapTarget?.grid ?? "");
  ...
  const columns = useMemo(() => location && isValidGrid(targetGrid) && solarCurrent && model.current.kp && model.current.flux ? buildHomeForecast({ origin: location, target: gridToLatLon(targetGrid), now, kp: model.current.kp.kp, sfi: model.current.flux.flux, predictedKp: model.current.predictedKp, fluxForecast: model.resources.forecast.state === "fresh" || model.resources.forecast.state === "refreshing" ? model.resources.forecast.data : undefined, mode, power: Number(power), antenna, noise }) : [], [location,targetGrid,solarCurrent,model,now,mode,power,antenna,noise]);
