...
export function HomeLocationPicker() {
  ...
  const close = () => { locationRequest.current += 1; setLocating(false); setOpen(false); setError(""); };
  const apply = (value: string) => { choose(value); close(); };
  const locate = () => {
    if (!navigator.geolocation) { setError("Location is unavailable in this browser. Enter your grid instead."); return; }
    const request = ++locationRequest.current;
    setLocating(true);
    setError("");
    navigator.geolocation.getCurrentPosition(pos => {
      if (request !== locationRequest.current) return;
      apply(latLonToGrid(pos.coords.latitude, pos.coords.longitude, 6));
    }, () => {
      if (request !== locationRequest.current) return;
      setLocating(false);
      setError("Your location could not be read. Enter your grid instead.");
  ...
  return <><button type="button" aria-label={location ? `Choose Home location: ${location.grid}` : "Set your location"} onClick={() => { setGrid(location?.grid ?? ""); setOpen(true); }}> {location ? `Location: ${location.grid}` : "Set your location"} ⌄</button>
    <AccessibleDialog open={open} onClose={close} title="Choose your location" description="Use a grid square without creating an account.">
      ...
      </form><button type="button" disabled={locating} onClick={locate}>{locating ? "Finding location…" : "Use my approximate location"}</button><p className="home-note">Saved in this browser. This changes Home’s location, not your station setup.</p></div>
    </AccessibleDialog></>;
}
