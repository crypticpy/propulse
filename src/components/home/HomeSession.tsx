...
export function HomeSession({ now, isMobile }: { now: number; isMobile: boolean }) {
  ...
  return <section aria-label="Your recent operating" className="home-panel home-session su-section-panel">
    <div aria-hidden="true" className="su-section-rule" />
    <SectionHeader as="h3" className="su-widget-header" title="Your recent operating" summary="Contacts logged in the last seven UTC days." action={<HomeStatus state={log.loading ? "loading" : log.error ? "error" : "local"} />} />
