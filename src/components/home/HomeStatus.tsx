...
export function HomeStatus({ state }: { state: SolarWidgetState | "local" }) {
  ...
  return <span role="status" className="inline-flex shrink-0 items-center gap-1.5 text-xs text-slate-400"><span aria-hidden="true" className={`h-1.5 w-1.5 rounded-full ${state === "fresh" ? "bg-signal-green" : state === "stale" || state === "partial" ? "bg-caution-amber" : state === "error" ? "bg-alert-red" : "bg-slate-500"}`} />{label}</span>;
}
