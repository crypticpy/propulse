/**
 * Location sub-text-xs guard — batch 44 (#808)
 *
 * Census on `origin/main`: remaining user-read location components with
 * sub-floor `text-[Npx]` (N<12). This batch raises 1 user-read site in
 * QuickLocationControl.tsx (grid variant label). Skips phone breakpoints.
 *
 * Sibling to other `subTextSizeFloor.*.test.ts` guards — does not edit them.
 */
...
const FILES = ["src/components/location/QuickLocationControl.tsx"];
...
describe("sub-text-xs sizing stays at the floor in location (#808 batch 44)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-44 location files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
