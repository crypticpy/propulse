...
export function DateTimePicker({
  ...
  return (
    <div
      className={`bg-su-canvas/95 backdrop-blur-md rounded-lg border border-su-line/40 shadow-xl p-3 ${className}`}
    >
      {/* Month Navigation */}
          ...
          return (
            <button
              ...
              className={`
                ...
                ${
                  isSelected
                    ? "bg-plasma-orange text-su-on-accent font-bold"
                    : isToday
      ...
      {/* Time Input */}
      <div className="border-t border-su-line/40 pt-3 mt-3">
        <div className="flex items-center justify-center gap-2">
          <label className="text-xs text-su-muted">UTC Time:</label>
          <input
            type="number"
            min={0}
            max={23}
            ...
            className="w-12 px-2 py-1 text-center text-sm bg-su-line/10 border border-su-line/40 rounded text-su-text focus:outline-none focus:border-plasma-orange"
          ...
          />
          <span className="text-xs text-su-muted">UTC</span>
        </div>

        {/* Local time display */}
        <div className="text-center mt-2 text-xs text-su-muted">
          Local: {format(selectedDate, use24h ? "HH:mm" : "h:mm a")}
        </div>
      </div>

      {/* Quick Actions */}
      <div className="flex gap-2 mt-3 border-t border-su-line/40 pt-3">
        <button
          onClick={() => {
            const now = new Date();
            setSelectedDate(now);
        ...
        <button
          onClick={onClose}
          className="flex-1 px-2 py-1.5 text-xs font-medium bg-su-line/10 text-su-muted border border-su-line/40 rounded hover:bg-su-line/20 transition-colors"
        >
          Done
        </button>
      </div>
