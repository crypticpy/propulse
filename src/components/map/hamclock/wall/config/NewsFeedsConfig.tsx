...
export function NewsFeedsConfig() {
    ...
    content: <div>{feeds.slice(page * 2, page * 2 + 2).map(feed => {
      ...
      return <div key={feed.id}>
        <HamClockToggleRow label={feed.label} icon={<span>RSS</span>}
          ...
          actions={<><HamClockButton disabled={result?.isFetching} onClick={() => void result?.refresh()}>REFRESH</HamClockButton><HamClockButton disabled={feeds.length <= 1} onClick={() => removeFeed(feed.id)}>REMOVE</HamClockButton></>} />
  ...
  return <HamClockTabs label="News configuration" tabs={[
    ...
    { id: "add", label: "ADD FEED", content: <div>
      ...
      <div><HamClockButton disabled={pending || !url.trim() || feeds.length >= MAX_FEEDS} onClick={() => void verify()}>{pending ? "VERIFYING" : "VERIFY"}</HamClockButton>
        <HamClockButton disabled={!verified || verified.url !== url.trim() || feeds.length >= MAX_FEEDS} onClick={() => {
          ...
          if (addFeed(verified.url, verified.title)) { setUrl(""); setVerified(null); setMessage("Feed added."); }
...
export function NewsFeedsConfigDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
