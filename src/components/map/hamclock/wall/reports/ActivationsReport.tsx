...
import { ACTIVATION_PROGRAMS, ACTIVATION_PROGRAM_META, type ActivationProgram, type ActivationSpot } from "@/types/activationSpots";
...
export function ActivationsReport({ open, onClose, initialProgram = "POTA" }: {
  ...
  return <WallReport open={open} onClose={onClose} title="Activations report" tone={state === "CURRENT" ? "accent" : "warn"}
    ...
    facts={[
      { label: "LOADED · 2 H", value: spots.length },
...
function ActivationRows({ spots, now, loading, state }: { spots: ActivationSpot[]; now: number; loading: boolean; state: string }) {
  const [ref, visible] = useVisibleRows<HTMLDivElement>(spots.length);
  return <div className="hca-report">
    <p className="hcr-note">{state === "CURRENT" ? "Reported within 2 h; a spot does not confirm the station is still on air." : `${state} · retained reports may no longer be active.`}</p>
    <div className="hca-list" ref={ref}>
      {spots.slice(0, visible).map((spot) => <div className="hca-row" key={`${spot.program}:${spot.callsign}:${spot.reference}`}>
        ...
        <div className="hca-detail"><span>{bandFromFreq(spot.frequencyKHz) ?? "—"} · {spot.mode || "UNKNOWN"}</span><span>{activationAge(spot.spottedAt, now)} AGO · {spot.spotter || "SPOTTER UNKNOWN"}</span></div>
    ...
    <div className="sr-only"><table aria-label="All loaded activations"><thead><tr>{["Callsign", "Reference", "Name", "Frequency kHz", "Mode", "Reported UTC", "Spotter"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{spots.map((spot) => <tr key={`${spot.program}:${spot.callsign}:${spot.reference}`}><td>{spot.callsign}</td><td>{spot.reference}</td><td>{spot.referenceName}</td><td>{spot.frequencyKHz}</td><td>{spot.mode}</td><td>{spot.spottedAt}</td><td>{spot.spotter}</td></tr>)}</tbody></table></div>
  </div>;
}
