...
export function BandActivityReport({ open, onClose, initialGlobalCounts = false, initialView = "bands" }: BandActivityReportProps) {
  ...
  const latestClusterSpot = useMemo(
    () =>
      clusterSpots.reduce<number | undefined>((latest, spot) => {
        const timestamp = new Date(spot.time).getTime();
        return Number.isFinite(timestamp) && (latest === undefined || timestamp > latest)
          ? timestamp
          : latest;
      }, undefined),
    [clusterSpots],
  ...
  const reportFacts: WallReportFact[] = isTopDx
    ? [
        { label: "LOADED SPOTS", value: clusterSpots.length },
        { label: "SOURCE", value: clusterSource.toUpperCase() },
        { label: "WINDOW", value: "UP TO 60 MIN" },
      ]
    : facts;
  const { footer, updated } = reportFooter(
    isTopDx
      ? `DX CLUSTER · ${clusterSource.toUpperCase()} · ${clusterSpots.length ? "LOADED" : "EMPTY"} · LAST SPOT`
      : "LIVE ACTIVITY · RAW 60 MIN / DEDUPED MODES 20 MIN",
    isTopDx ? latestClusterSpot : data?.fetchedAt,
  );
  ...
  return (
    <WallReport
      open={open}
      onClose={onClose}
      title={`Band activity report · ${isTopDx ? "TOP DX FROM HOME" : scope.label}`}
      tone="accent"
      hero={
        isTopDx ? (
          // Current main requires a hero and always renders the lead. The
          // marker lets this existing-file integration select a body-only
          // layout without pulling in the unmerged shared-shell API change.
          <span className="hcr-top-dx-hero">TOP DX</span>
        ) : top ? (
          top.band.toUpperCase()
        ) : (
          "—"
        )
      }
      ...
      pinElement={<BandActivityReport open onClose={onClose} initialGlobalCounts={globalCounts} initialView={activeView} />}
    >
      {!isTopDx && activeActivityScope.type !== "global" && (
