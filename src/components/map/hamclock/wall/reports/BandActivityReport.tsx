...
export function BandActivityReport({ open, onClose, initialGlobalCounts = false, initialView = "bands" }: BandActivityReportProps) {
  ...
  const { footer, updated } = reportFooter(
    isTopDx ? `DX CLUSTER · LOADED SAMPLE · ${clusterFeed.state}` : "LIVE ACTIVITY · RAW 60 MIN / DEDUPED MODES 20 MIN",
    isTopDx ? (clusterSource === "bridge" ? clusterFeed.observedAt : clusterFeed.fetchedAt) ?? undefined : data?.fetchedAt,
  );
  ...
  return (
    <WallReport
      ...
      tone="accent"
      hero={isTopDx ? undefined : top ? top.band.toUpperCase() : "—"}
      verdict={
        isTopDx ? undefined : isError ? "FEED OFF" : isPending ? "WAITING" : top ? "LEADS" : "QUIET"
      }
      facts={isTopDx ? [] : facts}
      footer={footer}
      updated={updated}
