...
it("retains cluster content inside report chrome without inventing a hero or read time", () => {
  ...
  expect(screen.getByText("Existing cluster filters and content")).toBeTruthy();
  expect(screen.getByText(/DX CLUSTER REST · LAST SPOT/)).toBeTruthy();
  expect(screen.getByRole("button", { name: "PIN" })).toBeTruthy();
});
it("pins the existing list across owner unmount and unpins cleanly", () => {
  function Harness() {
