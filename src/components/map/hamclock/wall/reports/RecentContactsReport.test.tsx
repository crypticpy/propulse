...
it("tunes the logged last and best contacts with their precise frequency and mode", async () => {
  mocks.read.mockResolvedValue({ entries: [{ ...contact, frequency: 14074.125, mode: "CW" }], totalCount: 1, readAt: Date.now() });
  draw();
  const buttons = await screen.findAllByRole("button", { name: "Tune 14.074125 MHz CW" });
...
it("does not infer a tune frequency from the logged band", async () => {
  mocks.read.mockResolvedValue({ entries: [{ ...contact, frequency: 0 }], totalCount: 1, readAt: Date.now() });
  draw();
  const buttons = await screen.findAllByRole("button", { name: /Tune.*INVALID FREQUENCY/ });
  for (const button of buttons) {
