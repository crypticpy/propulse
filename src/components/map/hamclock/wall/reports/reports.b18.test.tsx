...
it("preserves the selected band and hour when the report is pinned", () => {
  function Harness() {
    const [open, setOpen] = useState(true);
    return <>{open && <ReliabilityReport open onClose={() => setOpen(false)} />}<HamClockPinnedReportHost /></>;
  }
  render(<Harness />);
  fireEvent.click(screen.getByRole("tab", { name: "BY HOUR" }));
  fireEvent.click(screen.getByRole("button", { name: "40m 06Z: 60 of 100" }));
  fireEvent.click(screen.getByRole("button", { name: "PIN" }));
  expect(screen.getByText("40M", { selector: ".hcr-hero" })).toBeTruthy();
  expect(screen.getByRole("tab", { name: "BY HOUR" }).getAttribute("aria-selected")).toBe("true");
  expect(screen.getByRole("button", { name: "40m 06Z: 60 of 100" }).getAttribute("aria-pressed")).toBe("true");
