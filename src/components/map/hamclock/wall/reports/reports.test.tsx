...
it.each(["rest", "bridge"] as const)("TOP DX uses its own %s source timestamp and context", (source) => {
  ...
  useDXStore.setState({ spots: [], spotSource: source, clusterFeed: {
    state: "STALE", windowMinutes: 30,
    fetchedAt: Date.parse("2026-09-05T12:00:00Z"),
    observedAt: Date.parse("2026-09-05T11:30:00Z"),
  } });
  try {
    ...
    expect(screen.getByText(/DX CLUSTER · LOADED SAMPLE · STALE/)).toBeTruthy();
    expect(screen.getByText(source === "bridge" ? /11:30 UTC/ : /12:00 UTC/)).toBeTruthy();
