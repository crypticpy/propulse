...
export function ForecastMatrixTile({
  ...
  const report = reportOpen ? <Suspense fallback={null}><ForecastReport open onClose={() => setReportOpen(false)} /></Suspense> : null;
  ...
  if (status !== "ready" || !hero) {
    return (
      <><HamClockTile title={title} onOpen={() => setReportOpen(true)} openLabel="Open forecast report">
