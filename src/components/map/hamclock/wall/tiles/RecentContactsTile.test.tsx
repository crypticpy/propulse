...
vi.mock("@/lib/hamclock/recentContacts", () => ({
  readHamClockContacts: mocks.read,
}));
vi.mock("@/hooks/useActiveLocation", () => ({
  useActiveLocation: () => ({ grid: "EM38" }),
}));
...
it("tunes an asynchronously loaded row without opening the report", async () => {
  ...
  const tune = await screen.findByRole("button", {
    name: "Tune 7.074125 MHz LSB",
  });
