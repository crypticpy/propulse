/**
 * Map sub-text-xs guard — batch 25 HUD (#808)
 *
 * Census on `origin/main`: `src/components/map/` holds sub-floor `text-[Npx]`
 * sites (N<12) in HUD/label chrome outside concurrent PR #1196/#1199 files,
 * hamclock/, ProToolbarRibbon.tsx, WatchPopover.tsx, layers/SatMatchPanel.tsx,
 * and 3D canvas overlay chrome. This batch raises 27 user-read sites in the
...
const FILES = [
  "src/components/map/ActiveKitChip.tsx",
  "src/components/map/BandConditionsHeader.tsx",
  "src/components/map/ClusterPopover.tsx",
  "src/components/map/CompassRose.tsx",
  "src/components/map/CorrelationIndicator.tsx",
  "src/components/map/FullscreenPropSphere.tsx",
  "src/components/map/KeyboardShortcutsOverlay.tsx",
  "src/components/map/LabelsPanel.tsx",
  "src/components/map/MapToolbarShell.tsx",
  "src/components/map/MapTooltip.tsx",
  "src/components/map/NVISAnalysis.tsx",
  "src/components/map/ReplayIndicator.tsx",
  "src/components/map/StyleSelector.tsx",
  "src/components/map/ViewsPopover.tsx",
];
...
describe("sub-text-xs sizing stays at the floor in map (#808 batch 25 HUD)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-25 HUD files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
...
it("keeps the measured tooltip inside the viewport after resize", async () => {
  const { createElement } = await import("react");
  const { render, act } = await import("@testing-library/react");
  const { MapTooltip } = await import("./MapTooltip");
  vi.stubGlobal("innerWidth", 390);
  vi.stubGlobal("innerHeight", 900);
  vi.stubGlobal("ResizeObserver", class { observe() {} disconnect() {} });
  const measure = vi.spyOn(HTMLElement.prototype, "getBoundingClientRect").mockReturnValue({ width: 220, height: 225 } as DOMRect);
  const view = render(createElement(MapTooltip, { visible: true, position: { x: 370, y: 880 }, grid: "EN50aa" }));
  try {
    const tooltip = view.baseElement.querySelector<HTMLElement>(".fixed")!;
    expect(tooltip.style.top).toBe("645px");
    vi.stubGlobal("innerHeight", 600);
    act(() => window.dispatchEvent(new Event("resize")));
    expect(tooltip.style.top).toBe("365px");
    expect(tooltip.style.left).toBe("135px");
  } finally {
