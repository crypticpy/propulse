    ...
    <div className="relative flex items-center gap-2" data-callsign-input>
      <div className="relative flex-1">
        <input
          ...
          className="w-full bg-su-line/10 border border-su-line/50 rounded-xl text-base py-3 px-4 text-su-text font-mono placeholder:text-su-muted focus:outline-none focus:ring-2 focus:ring-plasma-orange/70 focus:border-plasma-orange/50 focus:bg-su-line/20 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
        />
        ...
        {isOpen && (
          <div
            ref={dropdownRef}
            role="listbox"
            className="absolute left-0 right-0 top-full mt-1 z-50 bg-deep-space/95 backdrop-blur-md border border-su-line/50 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-top-1"
          >
            {suggestions.map((cs, i) => (
              <div
                ...
                className={`py-2.5 px-4 text-sm font-mono cursor-pointer transition-colors ${
                  i === highlightIndex
                    ? "bg-su-line/30 text-su-text border-l-2 border-l-plasma-orange"
                    : "text-su-muted hover:bg-su-line/20 border-l-2 border-l-transparent"
                }`}
              >
