/**
 * Operating sub-text-xs guard (#808 batch 28)
 *
 * Census on `origin/main`: `src/components/operating/` holds sub-floor
 * `text-[Npx]` sites (N<12) across BandModeModalContent, BandModeSelector,
 * and BandSuggestToast. This batch raises user-read copy to `text-xs`.
 *
...
const OPERATING_ROOT = resolve(REPO_ROOT, "src/components/operating");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = [
  "src/components/operating/BandModeModalContent.tsx",
  "src/components/operating/BandModeSelector.tsx",
  "src/components/operating/BandSuggestToast.tsx",
];
...
function walkOperatingSourceFiles(dir: string): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    if (statSync(abs).isDirectory()) {
      results.push(...walkOperatingSourceFiles(abs));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in operating (#808 batch 28)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed operating files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-28 files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no un-allowlisted sub-floor text-[Npx] anywhere under src/components/operating", () => {
    const violations: string[] = [];
    for (const abs of walkOperatingSourceFiles(OPERATING_ROOT)) {
    ...
    expect(
      violations,
      `sub-floor sizing under operating/:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
