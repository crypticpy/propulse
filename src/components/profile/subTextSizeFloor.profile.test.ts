/**
 * Profile sub-text-xs guard (#808 batch 29)
 *
 * Census on `origin/main`: `src/components/profile/` holds sub-floor
 * `text-[Npx]` sites (N<12) across twelve profile components in this batch.
 * This batch raises user-read copy to `text-xs`.
 *
 * Skipped (other batches / constraints): CallsignLookupSuggestions,
 * StationIdentityForm, ProfilePage (#1171), ActivityFeed, LicenseCard.
 */
...
const PROFILE_ROOT = resolve(REPO_ROOT, "src/components/profile");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = [
  "src/components/profile/BandConditionsBar.tsx",
  "src/components/profile/ContactThisStation.tsx",
  "src/components/profile/FavoriteFreqList.tsx",
  "src/components/profile/InterestTagDisplay.tsx",
  "src/components/profile/InterestTagPicker.tsx",
  "src/components/profile/OnAirToggle.tsx",
  "src/components/profile/PrivilegeMatrix.tsx",
  "src/components/profile/ProfileCard.tsx",
  "src/components/profile/ScheduleOverlap.tsx",
  "src/components/profile/ShareCard.tsx",
  "src/components/profile/VisitorProfileCard.tsx",
  "src/components/profile/WhereToFindMe.tsx",
];
...
function walkProfileSourceFiles(dir: string): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    if (statSync(abs).isDirectory()) {
      results.push(...walkProfileSourceFiles(abs));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in profile (#808 batch 29)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed profile files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-29 files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no un-allowlisted sub-floor text-[Npx] in the fixed profile files (directory scan)", () => {
    const fixedSet = new Set(FILES);
    const violations: string[] = [];
    for (const abs of walkProfileSourceFiles(PROFILE_ROOT)) {
      const rel = abs.slice(REPO_ROOT.length + 1);
      if (!fixedSet.has(rel)) continue;
    ...
    expect(
      violations,
      `sub-floor sizing in fixed profile files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
