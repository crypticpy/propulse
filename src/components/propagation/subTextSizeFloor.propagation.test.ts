/**
 * Propagation sub-text-xs guard (#808 batch 34)
 *
 * Census on `origin/main`: `src/components/propagation/` holds 11 sub-floor
 * `text-[Npx]` sites (N<12) in NowCastBandPanel.tsx and
 * ResearchAttemptControl.tsx. This batch raises user-read copy to `text-xs`.
 *
 * Skips Home (DashboardSection), Solar (SolarPulseSection), and phone-only
 * layouts (none present in these files).
 *
...
const PROPAGATION_ROOT = resolve(REPO_ROOT, "src/components/propagation");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = [
  "src/components/propagation/NowCastBandPanel.tsx",
  "src/components/propagation/ResearchAttemptControl.tsx",
];
...
function walkPropagationSourceFiles(dir: string, relPrefix = ""): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    const rel = relPrefix ? `${relPrefix}/${entry}` : entry;
    if (statSync(abs).isDirectory()) {
      results.push(...walkPropagationSourceFiles(abs, rel));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in propagation (#808 batch 34)", () => {
  it("has no sub-floor text-[Npx] or inline fontSize in the fixed propagation files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-34 propagation files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no sub-floor text-[Npx] anywhere under src/components/propagation", () => {
    const violations: string[] = [];
    for (const abs of walkPropagationSourceFiles(PROPAGATION_ROOT)) {
    ...
    expect(
      violations,
      `sub-floor sizing under propagation/:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
});
