/**
 * Radio sub-text-xs guard (#808 batch 26)
 *
 * Census on `origin/main`: `src/components/radio/` holds 14 sub-floor
 * `text-[Npx]` sites (N<12) in RadioPickerModal.tsx — equipment IDs, tier
 * metadata, badges, and action buttons. This batch raises user-read copy to
 * `text-xs`.
 *
...
const RADIO_ROOT = resolve(REPO_ROOT, "src/components/radio");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = ["src/components/radio/RadioPickerModal.tsx"];
...
function walkRadioSourceFiles(dir: string): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    if (statSync(abs).isDirectory()) {
      results.push(...walkRadioSourceFiles(abs));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in radio (#808 batch 26)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed radio files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-26 files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no un-allowlisted sub-floor text-[Npx] anywhere under src/components/radio", () => {
    const violations: string[] = [];
    for (const abs of walkRadioSourceFiles(RADIO_ROOT)) {
