/**
 * Rank sub-text-xs guard (#808 batch 35b)
 *
 * Raises user-read sub-floor `text-[Npx]` (N<12) to `text-xs` in rank
 * components.
 *
...
const FILES = [
  ...
  "src/components/rank/LegendaryEffects.tsx",
];
...
describe("sub-text-xs sizing stays at the floor in rank (#808 batch 35b)", () => {
  it("has no sub-floor text-[Npx] or inline fontSize in the fixed rank files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-35b rank files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
});
