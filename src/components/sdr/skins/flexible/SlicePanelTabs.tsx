  ...
  return (
    <div className="space-y-2">
      ...
      {supports("rit") && (
        ...
        />
        <span className="text-xs font-mono text-su-muted w-14 text-right tabular-nums">
          {ritOffset >= 0 ? "+" : ""}
          {ritOffset}
        </span>
        {ritEnabled && ritOffset !== 0 && (
          <button
            ...
            disabled={!canControl}
            className="text-xs text-su-muted hover:text-su-text disabled:opacity-40"
      ...
      {supports("if_shift") && (
        <div className="flex items-center gap-1.5">
        <span className="text-xs text-su-muted shrink-0 w-7">IF</span>
        <input
          type="range"
          min={-2000}
          max={2000}
      ...
      {isCw && supports("cw_speed") && (
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-su-muted shrink-0 w-7">WPM</span>
          <input
            type="range"
            min={5}
            max={60}
          ...
          />
          <span className="text-xs font-mono text-su-muted w-8 text-right tabular-nums">
            {cwSpeed}
          </span>
        </div>
      )}
