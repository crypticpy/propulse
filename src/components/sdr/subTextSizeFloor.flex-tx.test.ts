/**
 * SDR sub-text-xs guard — batch 58 Flex TX chip (#808)
 *
 * Census on `origin/main`: `FlexBottomBar.tsx` has one remaining user-read
 * sub-floor site — the TX badge (`text-[10px]`). Every other label in the
 * bottom bar was already raised to `text-xs` in prior #808 slices. This
 * batch fixes that badge only; axis tick labels and phone chrome stay out
 * of scope.
 *
 * Sibling to the other SDR floor tests — does not edit them while concurrent
 * PRs (#1242 pulseTextContrast, batch-4 allowlist) are open.
 */
...
describe("sub-text-xs sizing stays at the floor in SDR (#808 batch 58 flex-tx)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-58 flex-tx files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
