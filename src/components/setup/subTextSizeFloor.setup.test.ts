/**
 * Setup sub-text-xs guard (#808 batch 31)
 *
 * Census on `origin/main`: `src/components/setup/` holds 11 sub-floor
 * `text-[Npx]` sites (N<12) in ArchitectureDiagram.tsx. This batch raises
...
const SETUP_ROOT = resolve(REPO_ROOT, "src/components/setup");

/** Files this batch read and fixed — append-only for follow-ups. */
const FILES = ["src/components/setup/ArchitectureDiagram.tsx"];
...
function walkSetupSourceFiles(dir: string, relPrefix = ""): string[] {
  const results: string[] = [];
  for (const entry of readdirSync(dir)) {
    const abs = join(dir, entry);
    const rel = relPrefix ? `${relPrefix}/${entry}` : entry;
    if (statSync(abs).isDirectory()) {
      results.push(...walkSetupSourceFiles(abs, rel));
    } else if (
      /\.(tsx|ts)$/.test(entry) &&
...
describe("sub-text-xs sizing stays at the floor in setup (#808 batch 31)", () => {
  it("has no sub-floor text-[Npx] or inline fontSize in the fixed setup files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-31 setup files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });

  it("has no sub-floor text-[Npx] anywhere under src/components/setup", () => {
    const violations: string[] = [];
    for (const abs of walkSetupSourceFiles(SETUP_ROOT)) {
