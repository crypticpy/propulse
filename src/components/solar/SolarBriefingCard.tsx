...
export function SolarBriefingCard({ briefing, scales, children }: { briefing: SolarBriefing; scales?: NoaaScalesProduct; children: ReactNode }) {
  ...
  return (
    <section aria-label="HF briefing" className={`rounded-2xl border bg-su-panel p-4 sm:p-6 ${briefing.tone === "impact" ? "border-su-danger/40" : "border-su-warning/25"}`}>
      ...
      {briefing.state !== "loading" && incomplete && <p className="mt-1 truncate text-[11px] leading-4 text-su-warning">Updates pending · {delayedLine}</p>}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-su-muted">
        <button type="button" onClick={() => setMoreOpen(!moreOpen)} aria-expanded={moreOpen} aria-controls="solar-briefing-context" className="min-h-11 rounded-lg px-3 text-su-info hover:bg-su-line/10">{moreOpen ? "Less context" : "Why this briefing?"}</button>
      ...
      {moreOpen && <div id="solar-briefing-context" className="mt-3 space-y-2 border-t border-su-line/40 pt-3 text-sm leading-relaxed text-su-text">
        ...
        {incomplete && <p className="text-xs leading-6 text-su-warning">{briefing.missing.length > 0 && `Fresh readings aren’t available yet for ${briefing.missing.join(", ")}. `}{briefing.delayed.length > 0 && `Updates are delayed for ${briefing.delayed.join(", ")}. `}Propulse checks again automatically; no setting needs changing. Unreported conditions remain uncertain.</p>}
      </div>}
