 ...
 * away, in an inline expansion rather than a modal or a flyout. The evidence
 * list (sources & observation times) is no longer rendered here — it still
 * lives in the briefing model for a future dedicated sources page.
 */
const TONES = {
...
const SCALE_CHIP_TONE = {
  neutral: { edge: "border-su-line/40", strong: "text-su-text" },
  watch: { edge: "border-su-warning/40", strong: "text-su-warning" },
  impact: { edge: "border-su-danger/50", strong: "text-su-danger" },
} as const;
      ...
      <div className="flex flex-col gap-3 p-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5 sm:p-4 xl:flex-nowrap">
        ...
        <div role="group" aria-label="Official NOAA scales" className="flex flex-wrap items-center gap-2 text-xs text-su-muted">
          {scaleChips.map(([code, label, value]) => {
            const chipTone = SCALE_CHIP_TONE[noaaScaleTone(value)];
            const chipTitle =
              value == null
                ? `${label} — not reported`
                : value > 0
                  ? `${label} scale ${code}${value} — in force`
                  : `${label} — none in force`;
            return (
              <span key={code} title={chipTitle} className={`${CHIP} ${chipTone.edge} bg-su-input`}>
                <strong className={`font-mono text-sm ${chipTone.strong}`}>{value == null ? `${code} —` : `${code}${value}`}</strong>
                {label}
              </span>
            );
          })}
        ...
        <div id="solar-briefing-detail" className="border-t border-su-line/40 px-3 pb-3 sm:px-4 sm:pb-4">
          <div className="mt-3 space-y-2">
            {shown.map((statement) => (
              <p
                key={statement.id}
                className={`flex gap-2.5 text-base leading-relaxed text-su-text ${statement.kind === "impact" ? "font-medium" : ""}`}
              >
                <span aria-hidden="true" className={`mt-1 shrink-0 text-sm leading-none ${tone.mark}`}>
                  {tone.glyph}
                </span>
            ...
            {!shown.length && (
              <p className="text-sm text-su-muted">
                Propulse is checking for current measurements. Your station settings are unaffected.
              </p>
            )}
            {remaining.length > 0 && (
              <div className="space-y-1.5 border-t border-su-line/20 pt-2 text-sm leading-6 text-su-muted">
                {remaining.map((statement) => (
          ...
          <p className="mt-3 text-xs text-su-muted">Global conditions describe the backdrop. A contact depends on both stations and the path.</p>
          {children}
        </div>
      )}
    </section>
