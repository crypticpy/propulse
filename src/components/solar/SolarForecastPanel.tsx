...
export function SolarForecastPanel({ resources, current }: Pick<Model, "resources" | "current">) {
  return <>
          <div className="grid items-stretch gap-3 xl:grid-cols-[3fr_2fr]">
            <WidgetShell title="Planetary Kp timeline" eyebrow="Three-hour intervals · observed, estimated, predicted" {...sourceProps(resources.kp)} timestampLabel="Checked" observedAt={resources.kp.resource?.envelope.fetchedAt} className="h-full">
