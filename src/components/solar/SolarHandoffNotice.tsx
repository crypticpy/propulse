...
export function SolarHandoffNotice({ handoff, destination = "planner" }: { handoff: SolarHandoff | null; destination?: "planner" | "wizard" }) {
  ...
  return <p role="status" className="rounded-lg border border-cyan-300/20 bg-cyan-300/5 p-3 text-sm text-su-text">
    From {handoff.origin === "home" ? "Home" : "Solar Pulse"} · {handoff.target?.name ?? handoff.target?.grid ?? "Choose a target"} · {handoff.mode}{analysisMode !== handoff.mode ? ` (modeled with ${analysisMode} sensitivity)` : ""} · {handoff.at ? `Planning ${new Date(handoff.at).toUTCString()} with current solar inputs` : "Live conditions"}
  </p>;
}
...
export function SolarPlanningModeControl({ mode, onChange }: { mode: UIMode; onChange: (mode: UIMode) => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-3 p-3 text-sm text-su-text">
    <label className="flex items-center gap-3">Planning mode
      <select aria-label="Planning mode" value={mode} onChange={(event) => onChange(event.target.value as UIMode)} className="min-h-11 rounded-lg border border-su-line/50 bg-deep-space px-3 text-su-text">
