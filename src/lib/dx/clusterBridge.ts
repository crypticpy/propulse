...
/** Matches the bridge and daemon cluster parsers' minute-resolution tolerance. */
...
export function readClusterBridgeSpot(payload: unknown, now: number): DXSpot | null {
  ...
  if (![row.id, row.dx, row.spotter].every(value => typeof value === "string" && value.trim().length > 0 && value.length <= 256) ||
    ...
    typeof row.time !== "string" || !Number.isFinite(Date.parse(row.time)) || Date.parse(row.time) > now + CLUSTER_BRIDGE_FUTURE_TOLERANCE_MS ||
...
export function mergeClusterBridgeSpot(existing: readonly DXSpot[], incoming: DXSpot, maxAge: number | undefined, limit: number, now: number): DXSpot[] {
  ...
  return filterClusterAge([incoming, ...existing.filter(spot => spot.id !== incoming.id)], maxAge, now, CLUSTER_BRIDGE_FUTURE_TOLERANCE_MS)
