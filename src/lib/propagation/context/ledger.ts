 ...
 */
export const SOURCE_AGE_BOUNDS_SECONDS: Readonly<Record<string, number>> =
  Object.freeze({
    kp: 1800,
    magnetic_field: 1800,
    solar_wind: 1800,
...
export const SOURCE_LEDGER: Readonly<Record<string, SourceLedgerEntry>> =
  Object.freeze({
    kp: entry({
      sourceId: "kp",
      ...
      outageBehaviour:
        "feed keeps serving older bins; no activity flag is published",
    }),
    f107: entry({
      sourceId: "f107",
      ...
      licence: NOAA_LICENCE,
      outageBehaviour:
        "three measurements a day; a missed run leaves the previous value newest",
    }),
    sunspot_number: entry({
      ...
      licence: NOAA_LICENCE,
      outageBehaviour:
        "daily; the 45 day bound tolerates a long publication gap",
    }),
    ...
    dst: entry({
      ...
      licence: NOAA_LICENCE,
      outageBehaviour:
        "hourly; provisional values are revised, and a revision is a separate record",
    }),
    hp60: entry({
      sourceId: "hp60",
      ...
      outageBehaviour:
        "hourly index published about an hour after the hour it covers",
    }),
    kp_forecast: entry({
    ...
    f107_forecast: entry({
      ...
      licence: NOAA_LICENCE,
      outageBehaviour:
        "text product with an :Issued: header; a stale issue time is visible as one",
    }),
    outlook_27day: entry({
      ...
      licence: NOAA_LICENCE,
      outageBehaviour:
        "weekly text product with an :Issued: header and one row per day",
    }),
    r12_climatology: entry({
      ...
      archiveClass: "verified_as_issued",
      licence:
        "SILSO, free use with attribution to SILSO, Royal Observatory of Belgium",
      outageBehaviour:
...
export const LEDGER_SOURCE_IDS: readonly string[] = Object.freeze(
  Object.keys(SOURCE_LEDGER),
);
 ...
 */
export const DISABLED_CAPABILITIES: readonly DisabledCapability[] =
  Object.freeze([
    Object.freeze({
