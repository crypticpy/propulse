...
describe("what section 5.3.2 refuses rather than guesses", () => {
  ...
  it("closes the log-pole overflow that used to reach non_finite_result: item B's r12 and virtualSlantRangeKm envelopes refuse the combination outright", () => {
...
describe("the non_finite_result invariant on the assembled fL record", () => {
  ...
  it("cannot be reached through the public longPathLuf() surface once r12 and virtualSlantRangeKm are in envelope", () => {
    const result = call({
      r12: 1.5e308,
      virtualSlantRangeKm: LUF_PATH_CONSTANT_KM * (1 - 1e-5),
    });
