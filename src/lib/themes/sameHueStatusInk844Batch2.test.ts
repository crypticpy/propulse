/**
 * #844 batch 2: status-tint sites that batch 1 deferred.
 *
 * Same contract as the `STATUS_FIXED_SITES` table in
 * `accentTintContrast.test.ts` (batch 1, PR #1163): keep the status wash at
 * or below `/20`, draw labels in `--su-text`, never same-hue ink on the tint.
 * This sibling file certifies batch-2 sites only so it can land independently
 * of the open batch-1 PR.
 */

import { fileURLToPath } from "node:url";
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
import { describe, expect, it } from "vitest";
import { stationContrast, stationPalettes } from "@/lib/themes/stationTokens";
import type { ThemeId } from "@/lib/themes";

const AA = 4.5;
const TINT_CAP = 0.2;
const THEMES_IDS = Object.keys(stationPalettes) as ThemeId[];
const REPO_ROOT = resolve(fileURLToPath(import.meta.url), "../../../..");

type StatusToken =
  | "signal-green"
  | "caution-amber"
  | "alert-red"
  | "nebula-blue";

type StationPalette = (typeof stationPalettes)[ThemeId];

function compositeOnSurface(
  hex: string,
  alpha: number,
  surface: string,
): string {
  const channels = (value: string) =>
    [1, 3, 5].map((start) => parseInt(value.slice(start, start + 2), 16));
  const front = channels(hex);
  const back = channels(surface);
  return `#${front
    .map((channel, index) =>
      Math.round(channel * alpha + back[index] * (1 - alpha))
        .toString(16)
        .padStart(2, "0"),
    )
    .join("")}`;
}

const SURFACES = [
  { name: "panel", backdrop: (palette: StationPalette) => palette.panel },
  { name: "canvas", backdrop: (palette: StationPalette) => palette.canvas },
  {
    name: "glass over panel",
    backdrop: (palette: StationPalette) =>
      compositeOnSurface(palette.line, 0.1, palette.panel),
  },
  {
    name: "glass over canvas",
    backdrop: (palette: StationPalette) =>
      compositeOnSurface(palette.line, 0.1, palette.canvas),
  },
] as const;

interface Batch2Site {
  file: string;
  what: string;
  snippet: string;
  token: StatusToken;
}

function statusTintHex(palette: StationPalette, token: StatusToken): string {
  switch (token) {
    case "signal-green":
      return palette.success;
    case "caution-amber":
      return palette.warning;
    case "alert-red":
      return palette.danger;
    case "nebula-blue":
      return palette.panel;
  }
}

function deriveStatusAlpha(text: string, token: StatusToken): number {
  const re = new RegExp(`bg-${token}/(\\d+)`, "g");
  const alphas = [...text.matchAll(re)].map((m) => Number(m[1]));
  return Math.max(...alphas) / 100;
}

function assertNoSameHueInkOnTint(text: string, what: string, token: StatusToken): void {
  const tintRe = new RegExp(`bg-${token}/`);
  const sameHueInk = `text-${token}`;
  for (const line of text.split("\n")) {
    if (!tintRe.test(line) || !line.includes("text-su-text")) {
      continue;
    }
    expect(
      line.includes(sameHueInk),
      `${what}: same-line same-hue ink on ${token} tint:\n${line}`,
    ).toBe(false);
  }
}

const BATCH2_SITES: Batch2Site[] = [
  {
    file: "src/components/activation/ParkSearch.tsx",
    what: "the POTA Active badge in search results",
    snippet: `? "bg-signal-green/20 text-su-text"`,
    token: "signal-green",
  },
  {
    file: "src/components/nets/NetSessionHistory.tsx",
    what: "the expanded Summary toggle",
    snippet: `? "bg-nebula-blue/20 text-su-text border-nebula-blue/30"`,
    token: "nebula-blue",
  },
  {
    file: "src/components/nets/NetCard.tsx",
    what: "the next-session pill",
    snippet: `rounded-full bg-nebula-blue/10 text-su-text text-xs"`,
    token: "nebula-blue",
  },
  {
    file: "src/components/nets/NetCard.tsx",
    what: 'the "Newcomer Friendly" badge',
    snippet: `text-xs bg-signal-green/15 text-su-text rounded-full px-2 py-0.5"`,
    token: "signal-green",
  },
  {
    file: "src/components/nets/CloseoutPhase.tsx",
    what: "the Close Net Session button",
    snippet: `bg-alert-red/10 text-su-text border-2 border-alert-red/20 hover:bg-alert-red/20`,
    token: "alert-red",
  },
  {
    file: "src/components/nets/CloseoutPhase.tsx",
    what: "the session-complete icon circle",
    snippet: `bg-signal-green/10 border border-signal-green/20 flex items-center justify-center text-su-text"`,
    token: "signal-green",
  },
];

function assertOfflineBanner(source: string): void {
  expect(source.includes(`bg-caution-amber/20`)).toBe(true);
  expect(source.includes(`text-sm text-su-text"`)).toBe(true);
  expect(source.includes("text-caution-amber")).toBe(false);
}

describe("#844 batch 2 status-tint sites ship --su-text ink", () => {
  it("the offline banner draws labels in --su-text over the caution wash", () => {
    const source = readFileSync(
      resolve(REPO_ROOT, "src/components/nets/OfflineBanner.tsx"),
      "utf8",
    );
    assertOfflineBanner(source);
  });

  it.each(BATCH2_SITES.map((site) => [site.what, site] as const))(
    "%s still ships the measured snippet",
    (_what, site) => {
      const source = readFileSync(resolve(REPO_ROOT, site.file), "utf8");
      expect(
        source.includes(site.snippet),
        `${site.file} no longer contains:\n${site.snippet}`,
      ).toBe(true);
      assertNoSameHueInkOnTint(source, site.what, site.token);
      const alpha = deriveStatusAlpha(site.snippet, site.token);
      if (Number.isFinite(alpha) && alpha > 0) {
        expect(alpha, `${site.what} tint above cap`).toBeLessThanOrEqual(TINT_CAP);
      }
    },
  );

  it.each(
    BATCH2_SITES.flatMap((site) =>
      THEMES_IDS.map((theme) => [site.what, theme, site] as const),
    ),
  )(
    "%s clears AA in %s for --su-text on its status tint",
    (_what, theme, site) => {
      const alpha = deriveStatusAlpha(site.snippet, site.token);
      if (!Number.isFinite(alpha) || alpha <= 0) {
        return;
      }
      const palette = stationPalettes[theme];
      const tintHex = statusTintHex(palette, site.token);
      for (const surface of SURFACES) {
        const ratio = stationContrast(
          palette.text,
          compositeOnSurface(tintHex, alpha, surface.backdrop(palette)),
        );
        expect(
          ratio,
          `${site.file} on ${surface.name} at alpha ${alpha}`,
        ).toBeGreaterThanOrEqual(AA);
      }
    },
  );
});
