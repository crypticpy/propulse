...
import { applyThemeToDocument, getAccentPreset, getTheme, type ThemeId } from "./index";
...
const ACCENTS = [
  ...
  "#ffff00", "#00ffff", "#ff00ff", "#777777", "#767676",
  "#808080", "#010101", "#fefefe", "#ff6b35", "#c020ff",
  "#e020e0", "#a0e000", "#608020", "#123456", "#abcdef",
];
...
function rgb(hex: string): Rgb {
  if (!/^#[\da-f]{6}$/i.test(hex)) throw new Error(`Not a resolved hex: ${hex}`);
...
function minimum() {
  ...
  return {
    record(next: number, description: string) {
      if (next < ratio) { ratio = next; context = description; }
    },
...
describe("shared station treatment contrast", () => {
  it.each(THEMES)("keeps %s base, subtle and solid reading contracts across appearance settings", (theme) => {
    const baseMinimum = minimum();
    const restMinimum = minimum();
    const activeMinimum = minimum();
    const solidMinimum = minimum();
    const softInks = [stationPalettes.light.text, stationPalettes["high-contrast"].text];
    const failures: string[] = [];

    for (const mode of MODES) for (const saturation of SATURATIONS) for (const accent of ACCENTS) {
      ...
      if (tokens["--su-accent"] !== scaleHexChroma(accent, saturation)) {
        failures.push(`${context}: raw accent changed by solid resolution`);
      }
      for (const surface of ["canvas", "panel", "input"]) {
        baseMinimum.record(contrast(text, rgb(tokens[`--su-${surface}`])), `${context}/${surface}`);
      }
      for (const role of ROLES) {
        const raw = tokens[`--su-${role === "neutral" ? "muted" : role}`];
        const tone = rgb(raw);
        const label = `${context}/${role}`;
        restMinimum.record(contrast(text, mix(tone, STATION_TREATMENT_STRENGTH.rest, panel)), label);
        activeMinimum.record(contrast(text, mix(tone, STATION_TREATMENT_STRENGTH.active, panel)), label);
        ...
        if (!softInks.includes(ink)) failures.push(`${label}: noncanonical solid ink ${ink}`);
        const alreadyReadable = softInks.some((candidate) =>
          contrast(tone, rgb(candidate)) >= SOLID_TEXT_CONTRAST,
        );
        if (alreadyReadable && fill !== raw) failures.push(`${label}: changed already-readable fill`);
      }
    }
  ...
  it("emits treatment percentages without bogus RGB channel companions", () => {
    const tokens = stationTokens("dark", "#ff6b35");
    for (const [state, strength] of Object.entries(STATION_TREATMENT_STRENGTH)) {
      expect(tokens[`--su-treatment-${state}-strength`]).toBe(`${strength * 100}%`);
      expect(tokens[`--su-treatment-${state}-strength-rgb`]).toBeUndefined();
    }
    for (const role of ROLES) for (const part of ["fill", "ink"] as const) {
      const name = `--su-solid-${role}-${part}` as const;
      expect(tokens[`${name}-rgb`]).toBe(rgb(tokens[name]).join(" "));
    }
  });
  ...
  it("fits previously unreadable fills without introducing pure-white text", () => {
    ...
    for (const fill of ["#777777", "#c020ff", "#e400e4"]) {
      expect(Math.max(...softInks.map((ink) => contrast(rgb(fill), rgb(ink)))))
      ...
      expect(contrast(rgb(fitted.fill), rgb(fitted.ink))).toBeGreaterThanOrEqual(SOLID_TEXT_CONTRAST);
  ...
  it("distinguishes opaque rest backgrounds from extra tints on nested glass", () => {
    const tokens = stationTokens("dark", "#ffffff");
    const text = rgb(tokens["--su-text"]), tone = rgb(tokens["--su-accent"]);
    const panel = rgb(tokens["--su-panel"]), line = rgb(tokens["--su-line"]);
    const glass = mix(line, CARD_GLASS_ALPHA, panel);
    const hoveredGlass = mix(line, CARD_GLASS_ALPHA, glass);
    expect(contrast(text, mix(tone, STATION_TREATMENT_STRENGTH.rest, panel)))
      .toBeGreaterThanOrEqual(7);
    expect(contrast(text, mix(tone, STATION_TREATMENT_STRENGTH.rest, hoveredGlass)))
      .toBeLessThan(7);
  });

  it("keeps startup and fixed-dark fallback pairs consistent with the resolver", () => {
    const css = postcss.parse(readFileSync(
      resolve("src/styles/globals.css"), "utf8",
    ));
    const declarations = (selector: string) => {
      const rule = css.nodes.find((node): node is Rule =>
    ...
    for (const role of ROLES) for (const part of ["fill", "ink"] as const) {
      const name = `--su-solid-${role}-${part}` as const;
      expect(root[name]).toBe(tokens[name]);
      expect(root[`${name}-rgb`]).toBe(tokens[`${name}-rgb`]);
      if (role === "accent") {
  ...
  it("updates fixed-dark solid pairs with accessibility and saturation independently of the active theme", () => {
    ...
    try {
      for (const theme of THEMES) for (const mode of MODES) for (const saturation of SATURATIONS) {
        applyThemeToDocument(getTheme(theme), getAccentPreset("plasma"), mode, saturation);
        const dark = stationTokens("dark", "#ff6b35", mode, saturation);
        for (const role of ROLES) {
          if (role === "accent") continue;
          for (const part of ["fill", "ink"] as const) {
            expect(root.style.getPropertyValue(`--su-fixed-dark-solid-${role}-${part}`))
  ...
  it("connects the measured equations to CSS fills, state replacement and descendant ink", () => {
    const css = postcss.parse(readFileSync(
      resolve("src/components/station-ui/treatments.css"), "utf8",
    ));
    const rules: Rule[] = [];
    css.walkRules((rule) => { rules.push(rule); });
    function properties(selector: string) {
      const rule = rules.find((candidate) => candidate.selector === selector && candidate.parent?.type === "root");
    ...
    expect(base["--su-treatment-secondary-ink"]).toBe("var(--su-treatment-ink)");
    expect(properties(".su-treatment .su-treatment-secondary").color)
      .toBe("var(--su-treatment-secondary-ink)");
    ...
    css.walkDecls("--su-treatment-fill", (declaration) => {
      ...
      expect(declaration.value).toBe(equation("active"));
      activeSelectors.push((declaration.parent as Rule).selector);
    });
    expect(activeSelectors).toHaveLength(2);
    expect(activeSelectors.some((selector) => selector.includes(":hover"))).toBe(true);
    expect(activeSelectors.some((selector) => selector.includes('[aria-selected="true"]'))).toBe(true);
    expect(activeSelectors.every((selector) => selector.includes(":not(.su-treatment--solid)"))).toBe(true);
    ...
    for (const role of ROLES) {
      ...
      expect(tone["--su-tone"]).toBe(`var(--su-${role === "neutral" ? "muted" : role})`);
      expect(tone["--su-treatment-solid-fill"]).toBe(`var(--su-solid-${role}-fill)`);
      expect(tone["--su-treatment-solid-ink"]).toBe(`var(--su-solid-${role}-ink)`);
    }
    ...
    css.walkDecls((declaration) => {
      ...
      expect(["filter", "background-image", "box-shadow"]).not.toContain(declaration.prop);
    });
    const stationCss = postcss.parse(readFileSync(
      resolve("src/components/station-ui/station-ui.css"), "utf8",
    ));
