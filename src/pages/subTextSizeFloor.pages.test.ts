/**
 * Pages sub-text-xs guard (#808 batch 19)
 *
 * Census: `src/components/log/` is absent on origin/main, so this batch
 * targets `src/pages/` (excluding Home, SolarPulse, and *Mobile* pages).
 * 58 sub-floor `text-[Npx]` sites (N<12) across 11 page files — status
 * badges, section labels, satellite meta, net role chips, and health
 * dashboard copy. User-read copy raised to `text-xs`.
 *
 * Skips Home, SolarPulse, phone-only `text-[10px] sm:text-xs` on
 * FeaturesPage, and pages owned by other #808 batches (dx, contest, sdr,
 * kiosk, profile, map/PropSphere).
 *
...
const FILES = [
  "src/pages/Photorealistic3DPage.tsx",
  "src/pages/MapExplorerPage.tsx",
  "src/pages/BandPlanner.tsx",
  "src/pages/DisplaysPage.tsx",
  "src/pages/SatellitesPage.tsx",
  "src/pages/FeaturesPage.tsx",
  "src/pages/SystemHealthPage.tsx",
  "src/pages/SetupGuidePage.tsx",
  "src/pages/NetControllerPage.tsx",
  "src/pages/NetControllerDetailPage.tsx",
  "src/pages/NetAnalyticsPage.tsx",
];
...
describe("sub-text-xs sizing stays at the floor in pages (#808 batch 19)", () => {
  it("has no un-allowlisted sub-floor text-[Npx] or inline fontSize in the fixed page files", () => {
    ...
    expect(
      violations,
      `sub-floor sizing in batch-19 files:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
