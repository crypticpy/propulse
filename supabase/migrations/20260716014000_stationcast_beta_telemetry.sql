-- Aggregate-only StationCast beta API telemetry.
-- The table has no participant, request, path, location, or equipment columns.

CREATE TABLE IF NOT EXISTS public.propagation_beta_telemetry_hourly (
  protocol_version text NOT NULL,
  bucket_start timestamptz NOT NULL,
  requests bigint NOT NULL DEFAULT 0 CHECK (requests >= 0),
  errors bigint NOT NULL DEFAULT 0 CHECK (errors >= 0),
  integrity_errors bigint NOT NULL DEFAULT 0 CHECK (integrity_errors >= 0),
  privacy_events bigint NOT NULL DEFAULT 0 CHECK (privacy_events >= 0),
  consent_errors bigint NOT NULL DEFAULT 0 CHECK (consent_errors >= 0),
  subject_binding_errors bigint NOT NULL DEFAULT 0 CHECK (subject_binding_errors >= 0),
  stale_profile_events bigint NOT NULL DEFAULT 0 CHECK (stale_profile_events >= 0),
  equipment_math_events bigint NOT NULL DEFAULT 0 CHECK (equipment_math_events >= 0),
  unsupported_support_events bigint NOT NULL DEFAULT 0 CHECK (unsupported_support_events >= 0),
  high_confidence_overprediction_events bigint NOT NULL DEFAULT 0
    CHECK (high_confidence_overprediction_events >= 0),
  geographic_regression_events bigint NOT NULL DEFAULT 0
    CHECK (geographic_regression_events >= 0),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (protocol_version, bucket_start),
  CONSTRAINT propagation_beta_telemetry_bucket_hour_check CHECK (
    bucket_start = date_trunc('hour', bucket_start)
  )
);

ALTER TABLE public.propagation_beta_telemetry_hourly ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.propagation_beta_telemetry_hourly FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.propagation_beta_telemetry_hourly
  FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE
  ON TABLE public.propagation_beta_telemetry_hourly TO service_role;
...
BEGIN
  IF p_protocol_version IS NULL OR btrim(p_protocol_version) = ''
     ...
     OR p_window_start <> date_trunc('hour', p_window_start)
     OR p_window_end <> date_trunc('hour', p_window_end) THEN
...
REVOKE ALL ON FUNCTION public.record_propagation_beta_telemetry(
  text, timestamptz, jsonb
) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.get_propagation_beta_api_telemetry(
  text, timestamptz, timestamptz
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.record_propagation_beta_telemetry(
  text, timestamptz, jsonb
) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_propagation_beta_api_telemetry(
  text, timestamptz, timestamptz
) TO service_role;

COMMENT ON TABLE public.propagation_beta_telemetry_hourly IS
  'Hourly aggregate StationCast beta counters; participant, request, path, location, and equipment data are prohibited.';
COMMENT ON FUNCTION public.record_propagation_beta_telemetry(
  text, timestamptz, jsonb
) IS 'Records bounded aggregate-only beta counters through the service role.';
COMMENT ON FUNCTION public.get_propagation_beta_api_telemetry(
  text, timestamptz, timestamptz
) IS 'Exports the exact aggregate-only StationCast beta API telemetry receipt window.';
