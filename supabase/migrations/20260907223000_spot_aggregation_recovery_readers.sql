-- Preserve whichever reviewed reader contract is installed. Never create a
-- second overload: the deployed statistic argument has a default. Apart from
-- the gap predicate, each body preserves its existing rate/quantile semantics.
DO $migration$
DECLARE
  has_six boolean := to_regprocedure('public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text)') IS NOT NULL;
  has_seven boolean := to_regprocedure('public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text,text)') IS NOT NULL;
BEGIN
  IF has_six = has_seven THEN
    RAISE EXCEPTION 'expected exactly one supported lookup_path_recency_lags signature';
  END IF;
  IF has_six THEN
    EXECUTE $six$
...
BEGIN
  ...
  WITH targets AS (
    ...
    LEFT JOIN public.path_recency_hourly AS recency
     ...
     AND NOT EXISTS (
       SELECT 1 FROM public.collector_aggregation_gaps AS gap
...
$$;
$six$;
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text) FROM PUBLIC, anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text) TO service_role';
  ELSE
    EXECUTE $seven$
CREATE OR REPLACE FUNCTION public.lookup_path_recency_lags(p_issue_time timestamp with time zone, p_band text, p_origin_field text, p_target_fields text[], p_transform_version text, p_provider text, p_statistic text DEFAULT 'rate'::text)
 RETURNS TABLE(target_field text, path_success_prev1 double precision, path_success_prev2 double precision, path_success_prev3 double precision, path_success_prev24 double precision, path_prev1_available smallint, path_prev2_available smallint, path_prev3_available smallint, path_prev24_available smallint, source_watermark timestamp with time zone, available_at timestamp with time zone, provider text, transform_version text, quality_flags text[])
 LANGUAGE plpgsql
...
$function$;
$seven$;
    EXECUTE 'REVOKE EXECUTE ON FUNCTION public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text,text) FROM PUBLIC, anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.lookup_path_recency_lags(timestamptz,text,text,text[],text,text,text) TO service_role';
  END IF;
END;
$migration$;
